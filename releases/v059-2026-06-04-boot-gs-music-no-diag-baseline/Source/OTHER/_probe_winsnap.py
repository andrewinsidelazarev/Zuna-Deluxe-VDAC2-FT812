...
# На железе вход в геймплей идёт через EnterGameplayForCurrentLevel, который
# ставит slot3=#04 и CurrentCodePage=#04. Здесь LoadGameplayAssets зовётся
# напрямую, поэтому выставляем CurrentCodePage вручную: VDC_WinOutroInit
# дозаливает WIN-атлас через UnpackAndUploadPage, а та восстанавливает slot3
# именно в CurrentCodePage. Без этого slot3 уехал бы в UI-overlay (#41) и
# последующий прямой вызов DrawWinStateVisual (код overlay #04) попал бы не туда.
e.set_byte(S["CurrentCodePage"], 0x04)
for f in range(40):
