...
Новые baseline'ы (начиная **со следующего после 2026-05-18**) получают глобальный
номер `vNN`:

```
baseline_vNN_<descriptor>_<YYYY-MM-DD>/
zuma_vdac2_vNN_<descriptor>_<YYYY-MM-DD>.spg
```

- `vNN` — счётчик от **v01** по возрастанию. Не сбрасывается при смене даты,
  не привязан к git tag'у.
- `<descriptor>` — короткое имя что в этой версии нового.
- `<YYYY-MM-DD>` — дата создания (для контекста, не для сортировки).

Старые baseline'ы (`2026-05-10..2026-05-18`) сохраняют исторический формат
`baseline_<YYYY-MM-DD>_<descriptor>` без номера версии. Их не переименовываем —
ссылки в git-истории и в учебнике останутся валидны.
