...
.Loop           ; --- 0. Дождаться REG_DLSWAP=0 — predыдущий swap полностью завершён,
                ; DL buffer безопасно перезаписывать. Per FT81x PG: «Bit 1-0 of REG_DLSWAP
                ; — read=0 means DL buffer ready, otherwise host must wait till zero.»
                ; INT_SWAP fires raньше (= swap initiated), но hardware DLSWAP-clear
                ; происходит чуть позже. Без этого wait next frame's cmd_dlstart
                ; иногда стартует пока renderer ещё дорабатывает swap → flicker.
.WaitDLSwap     FT_RD_REG8 FT_REG_DLSWAP
                ...
                CALL ZL_KbdAimUpdate                  ; arrows ←/→ (SVK_5/8) → PositionX
                CALL ZL_SmoothMouse                   ; light low-pass

                ; --- 1a. Frog state (angle к курсору, recoil, blink) ---
                CALL Frog_Update

                ; --- 1b. VDC physics tick: TrySpawn + MoveChain + AnimateChain.
                CALL VDC_Update

                ; --- 2. Собрать DL в Z80-буфер CMD_ADDRESS_PTR ---
                FT_CMD_Start
                FT_DL_Start
                FT_VertexFormat 4                     ; явно 1/16 px (default может быть иной)
                ...
                ; --- WaitFlush ПЕРЕД DLSWAP — ждать пока coprocessor дочистит fifo,
                ; иначе DL ещё частично собран при swap'е → flicker. ---
...
; ----------------------------------------------------------------------------
; ZL_KbdAimUpdate — стрелки клавиатуры (Sinclair joystick keys 5=LEFT, 8=RIGHT)
; сдвигают Input.Mouse.PositionX на ZL_KBD_STEP px/кадр. Clamp [0..639].
; Spectrum keys; в Unreal эмуляторе PC arrow keys по умолчанию маппятся на 5/8.
; ----------------------------------------------------------------------------
ZL_KBD_STEP     EQU 6                                 ; px/frame ≈ 342 px/sec @57Hz

ZL_KbdAimUpdate:
                LD   A, Input.Keyboard.Spectrum.SVK_5
                CALL Input.Keyboard.Spectrum.KeyState
                JR   Z, .check_right
                LD   HL, (Input.Mouse.PositionX)
                LD   DE, ZL_KBD_STEP
                AND  A
                ...
                JR   Z, .store_left
                LD   HL, 0
.store_left:    LD   (Input.Mouse.PositionX), HL

.check_right:   LD   A, Input.Keyboard.Spectrum.SVK_8
                CALL Input.Keyboard.Spectrum.KeyState
                RET  Z
                LD   HL, (Input.Mouse.PositionX)
                LD   DE, ZL_KBD_STEP
                ...
                LD   DE, 640
                AND  A
                SBC  HL, DE                           ; HL = HL_new - 640
                JR   C, .right_ok                     ; HL_new < 640 → ОК
                LD   HL, -1                           ; clamp: после ADD HL,DE = 639
.right_ok:      ADD  HL, DE                           ; restore HL_new (или 639)
                LD   (Input.Mouse.PositionX), HL
                RET
