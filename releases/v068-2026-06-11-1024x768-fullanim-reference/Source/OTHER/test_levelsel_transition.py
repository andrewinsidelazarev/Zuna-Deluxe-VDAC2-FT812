...
def hook_safe_inflate(emu: ZumaFullZ80Emulator) -> None:
    ...
    def hooked_step():
        ...
        try:
            ...
            print(
                f"[inflate-fail] src_page=#{src_page:02X} src_offset=#{src_offset:04X} "
                f"dest=#{dest:06X} head={phys_slice[:8].hex()} err={exc}"
            )
            raise
...
def main() -> int:
    ...
    for name in (
        ...
        "FT.Coprocessor.Vertex2f",
        "FT.Coprocessor.ColorA",
    ...
    sky_addr = 0x04B000
    menu_sky = bytes(emu.ft.ram_g[sky_addr:sky_addr + 16])
    print(f"[RAM_G] menu sky @ #{sky_addr:06X}: {menu_sky.hex(' ')}")
    ...
    print(f"[RAM_G] level-select sky @ #{sky_addr:06X}: {sky_bytes_after.hex(' ')}")

    print()
    print("=== VERDICT ===")
    if sky_bytes_after == menu_sky:
        print("FAIL: level-select assets did not replace menu sky bytes in RAM_G")
        return 1
