...
PASS-критерий: между последним CMD_SETMATRIX до MENU и самим MENU действует
матрица c CMD_SCALE(0x1999A,0x1999A) (=1.6), и vertex = (862*16, 5*16).
"""
...
def main() -> int:
    ...
    for off, w in words(buf[menu_off:menu_off + 64]):
        if (w >> 30) == 1:  # VERTEX2F
            ...
            vertex = (x, y)
            break
    print(f"MENU vertex: {vertex} (ожидается {(862 * 16, 5 * 16)})")
    ...
    ok_scale = any(n == "SCALE" and a == [SCALE_16, SCALE_16] for n, a in applied)
    ok_vertex = vertex == (862 * 16, 5 * 16)
    if ok_scale and ok_vertex:
        print("PASS: MENU рисуется scale(1.6)-матрицей в (862,5) — код корректен")
        return 0
    print(f"FAIL: scale_ok={ok_scale} vertex_ok={ok_vertex}")
    return 1
