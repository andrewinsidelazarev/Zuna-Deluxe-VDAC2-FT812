...
FROG_SPR_HALF     EQU 61                                ; atlas center = UV pivot
...
FROG_SPR_SCR_HALF EQU 98                                ; round(195/2) — экранный центр (screen pivot)
FROG_SPR_SCR_HALF_NEG EQU -FROG_SPR_SCR_HALF & 0xFFFF
FROG_UPSCALE      EQU #0001999A                         ; cmd_scale 1.6 (f16.16) — общий апскейл лягушки
...
FROG_BALL_W       EQU 32                                ; ball atlas cell native classic 32×32 (layout)
...
; ----------------------------------------------------------------------------
; [Frog_ComputeAngle -> moved to shared_render.asm (resident; shared by gameplay #04 + level-select #41)]
; [Frog_Div16by8 -> moved to shared_render.asm (resident; shared by gameplay #04 + level-select #41)]
Frog_FireKeyboard:
...
; ----------------------------------------------------------------------------
; Frog_HandleMouse — edge-rise по Input_MouseLMB, при необходимости стартует fire.
...
.fh_block_save:   LD   (Frog_PrevMouseLeft), A
                  ...
                  RET
.fh_mouse_free:
                  ; Fire = только ЛКМ через Input.asm. RMB/SPACE/Kempston —
...
Frog_TickRecoil:
                  ...
                  ; ballExpand: if < FROG_BALL_IDLE → += 3 (HD 2.5/frame ×1.6 = 4 → 3 ближе к feel).
...
;   In: A = angle BRAD (caller добавил face-offset, как для cmd_rotate)
; ----------------------------------------------------------------------------
Frog_EmitFrogMatrix:
...
; Frog_Mul323Sh8 — In A=signed(-127..127), Out HL=signed A·1.25 = A+(A asr 2).
; scale 0.617 (валидировано sim_frog_matrix.py; ~1px по краю атласа, прозрачно).
Frog_Mul323Sh8:   LD   L, A : LD H, 0
                  BIT  7, A : JR Z, .p
                  DEC  H                               ; sign-extend (H=0xFF)
.p:               LD   D, H : LD E, L                  ; DE = sext(A)
                  SRA  H : RR L
                  SRA  H : RR L                        ; HL = A asr 2
                  ADD  HL, DE                          ; A + A/4 = A·1.25
                  RET
...
; Frog_EmitUpscale16 — append cmd_scale(1.6, 1.6). Один экземпляр, зовут
; body/overlay/tongue/plate. C/F пивота в матрице остаются корректны т.к.
; s·(draw/2) = atlas/2 (см. учебник §15.6).
Frog_EmitUpscale16:
                  LD   DE, FT_CMD_SCALE & #FFFF
                  LD   BC, (FT_CMD_SCALE >> 16) & #FFFF
                  CALL FT.Coprocessor.Command_BCDE
                  LD   DE, FROG_UPSCALE & #FFFF
                  LD   BC, (FROG_UPSCALE >> 16) & #FFFF
                  CALL FT.Coprocessor.Command_BCDE
                  LD   DE, FROG_UPSCALE & #FFFF
                  LD   BC, (FROG_UPSCALE >> 16) & #FFFF
                  JP   FT.Coprocessor.Command_BCDE
...
; ----------------------------------------------------------------------------
; [Frog_EmitRotateRaw -> moved to shared_render.asm (resident; shared by gameplay #04 + level-select #41)]
...
; ----------------------------------------------------------------------------
; [Frog_AtanTable -> moved to shared_render.asm (resident; shared by gameplay #04 + level-select #41)]
Frog_SinTable:
...
; ----------------------------------------------------------------------------
; [Frog_Angle -> moved to shared_render.asm (resident; shared by gameplay #04 + level-select #41)]
...
Frog_TmpSin:       DEFB 0                               ; baked-matrix scratch (sin127 signed)
...
; Frog_BallColor, Frog_NextBallColor -> hoisted to loader_resident.asm (resident Core)
Frog_PosX:         DEFW FROG_DEFAULT_X
Frog_PosY:         DEFW FROG_DEFAULT_Y
; [Frog_PosStartX -> moved to shared_render.asm (resident; shared by gameplay #04 + level-select #41)]
; [Frog_PosStartY -> moved to shared_render.asm (resident; shared by gameplay #04 + level-select #41)]
Frog_TmpX:         DEFW 0
Frog_TmpY:         DEFW 0
