...
LIFE_MAX_DRAW        EQU 3                             ; clamp displayed count (sock 125px fits 3×32)
HUD_MENU_X           EQU 862                           ; 539×1.6
HUD_MENU_Y           EQU 5                             ; 3×1.6
...
DrawKillzoneDual:
                ...
                CALL Core.ZL_EmitLoadId
                CALL Core.Frog_EmitUpscale16          ; 1024×768: scale 1.6 (88→141, scale-about-origin)
                CALL Core.ZL_EmitSetMatrix
                FT_BitmapHandle 3
...
DrawFrameStrips:
                ...
                LD   E, 255
                CALL FT.Coprocessor.ColorA
                ; 1024×768: одна cmd_scale(1.6)-матрица (scale-about-origin) на все 4
                ; рамки. Helper в #04 (Core), замаплен во время gameplay (зовёт MainLoop).
                CALL Core.ZL_EmitLoadId
                CALL Core.Frog_EmitUpscale16
                CALL Core.ZL_EmitSetMatrix
...
PAUSE_TITLE_CENTER_X EQU 455                            ; 512 − W×0.3: pause-путь центрует по W/2
                                                        ; (атлас), а экранная полуширина = W×0.8;
                                                        ; компенсация для str_pause_exit (W≈190)
PAUSE_TITLE_Y   EQU 246                                 ; пропорция: (154−60)×1.6 + 96 (рамка на 96)
PAUSE_YES_X    EQU 352                                  ; 220×1.6
PAUSE_NO_X     EQU 538                                  ; 336×1.6
PAUSE_BTN_Y    EQU 483                                  ; 302×1.6
PAUSE_BTN_W    EQU 134                                  ; 84×1.6
PAUSE_BTN_H    EQU 61                                   ; 38×1.6
PAUSE_YES_TEXT_X EQU 394                                ; 246×1.6
PAUSE_NO_TEXT_X  EQU 582                                ; 364×1.6
PAUSE_BTN_TEXT_Y EQU 490                                ; 306×1.6
...
.ds_loop:       LD   HL, (DrawStr_Ptr)
                ...
                ; --- Emit BITMAP_SIZE: окно глифа ШИРИНОЙ ТОЧНО w×8/5 (экранная при
                ; матрице 1.6). НЕ шире: атлас — непрерывная полоса глифов, окно
                ; больше w×scale захватывает соседнюю букву (артефакт-«палки»).
                ; Высота ×2 безопасна: layout height = FontHeight → UV ниже = BORDER.
                LD   H, 0
                LD   L, A                               ; HL = w
...
DrawHudTopText:
                ...
                LD   HL, DrawNumBuf
                CALL StrWidth
                LD   HL, 194                            ; center of left red socket (640-space)
                LD   A, E
                SRL  A                                  ; half width (atlas px)
                LD   E, A : LD D, 0
                AND  A
                SBC  HL, DE                             ; HL = x в 640-space
                CALL HudScaleXTo16                      ; ×8/5 → screen, ×16 subpx
                LD   B, H : LD C, L
                ...
                LD   HL, str_hud_score
                LD   BC, 434 * 16                       ; SCORE label pinned left (271×1.6)
                LD   DE, 2 * 16                         ; Y=2 (1×1.6)
                CALL DrawString
                ...
                LD   HL, DrawNumBuf
                CALL StrWidth
                LD   HL, 361                            ; number right edge (640-space)
                AND  A
                SBC  HL, DE                             ; HL = x в 640-space
                CALL HudScaleXTo16                      ; ×8/5 → screen, ×16 subpx
                LD   B, H : LD C, L
...
; HudScaleXTo16 — HL(640-space px) → HL = (HL×8/5)×16 (screen 1024, subpx 1/16).
