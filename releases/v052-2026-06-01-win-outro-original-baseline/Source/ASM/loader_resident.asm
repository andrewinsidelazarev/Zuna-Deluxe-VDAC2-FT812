...
WIN_PRTCL_MAX      EQU 24
WINEXP_MOVE        EQU 12               ; сэмплов/кадр (скорость бегущей точки)
WINEXP_PAD         EQU 50               ; сэмплов между взрывами (+100 за каждый)
...
; DrawLoadingScreen — full black frame with centered "LOADING LEVELS..." banner
; (pre-rendered nativealien48 48px ARGB4 pieces, тот же шрифт/градиент, что и LEVEL N-M).
; Картинка должна быть уже залита в LOADING_TEXT_RAMG (UploadLoadingText) ДО вызова.
; Строит один coprocessor-кадр и свопит его; кадр остаётся на экране во время
; блокирующего (DI) поиска PAK. Резидент в Core. Раньше — встроенный ROM-шрифт.
DrawLoadingScreen:
                FT_CMD_Start
                FT_DL_Start
                FT_VertexFormat 4
                ...
                FT_ClearAll
                FT_CMD_BUF #04FFFFFF            ; COLOR_RGB white (tint; градиент уже в картинке)
                FT_Begin FT_BITMAPS
                ...
                FT_Vertex2ii (640 - LOADING_LEVELS_W) / 2, (480 - LOADING_TEXT_H) / 2, LOADING_TEXT_HANDLE, 0
                ...
                FT_Vertex2ii ((640 - LOADING_LEVELS_W) / 2) + LOADING_TEXT_PREFIX_W, (480 - LOADING_TEXT_H) / 2, LOADING_TEXT_HANDLE, 0
                FT_End
                FT_Display
                FT_CMD_Count
