...
def canonical_tracks(folder: Path, level: int) -> list[Path]:
    direct = folder / f"{folder.name}-4-3.dat"
    if direct.exists():
        return [direct]
    first = folder / f"{folder.name}-1-4-3.dat"
    if first.exists():
        second = folder / f"{folder.name}-2-4-3.dat"
        if second.exists():
            return [first, second]
        return [first]
...
def build_bg_and_preview(level: int, src_png: Path) -> None:
    ...
    preview_src = crop_to_aspect(Image.fromarray(rgba, "RGBA"), PREVIEW_W, PREVIEW_H)
    preview = preview_src.resize((PREVIEW_W, PREVIEW_H), Image.Resampling.LANCZOS)
    ...
    write_pages(f"level_select_preview_l{level:02d}", preview_data, PREVIEW_PAGES)
...
def main() -> int:
    ...
    for folder in folders:
        ...
        tracks = canonical_tracks(folder, level)
        ...
        shutil.copy2(tracks[0], OUT / f"track_l{level:02d}_640.bin")
        second_out = OUT / f"track_l{level:02d}_2_640.bin"
        if len(tracks) > 1:
            shutil.copy2(tracks[1], second_out)
        elif second_out.exists():
            second_out.unlink()
        build_title(level, title)
        track_names = ",".join(t.name for t in tracks)
        print(f"L{level:02d}: {folder.name}  track={track_names}  title={title}")
