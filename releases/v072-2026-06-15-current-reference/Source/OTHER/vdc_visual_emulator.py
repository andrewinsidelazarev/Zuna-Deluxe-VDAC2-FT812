...
class VDCEngine:
    ...
    def check_match3(self, idx):
        ...
        # уезжает вперёд через дыру, пока gap ещё открыт.
        s.gap_accum = 0
        s.gap_dec_acc = 0
    ...
    def gap_junction_update(self):
        """= VDC_GapJunctionUpdate: тип стыка, который закроет do_gap_step
        (цель = его же правила: последний GAP_STOP, иначе первый CASCADE).
        0=гэпов нет / 1=PULL (цвета краёв рана равны) / 2=CATCH-UP."""
        s = self.s
        s.gap_junction = 0
        target = None
        ...
        while b > 0:
            ...
            if not is_gap(v):
                ...
                break
            b -= 1
        if left_color is None:           # фронт-сегмента нет → догон
            s.gap_junction = 2
            return
        # правый край рана (цвет тыла)
        d = target
        while d < s.slots_len - 1:
            d += 1
            v = s.slots[d]
            if is_gap(v):
                continue
    ...
    def gap_rear_comp(self, k, amount):
        """= VDC_GapRearComp: off[j] -= amount для ВСЕХ j от k до конца цепи
        (включая маркеры): индексный сдвиг удаления телепортит t каждого слота
        за стыком, а не только ближний сегмент — ранний выход на первом гэпе
        оставлял дальние сегменты с телепортом +32/шаг (рывки при нескольких
        дырах). Кламп к -CELL_SIZE."""
        s = self.s
        for j in range(k, s.slots_len):
            s.offsets[j] = max(-CELL_SIZE, s.offsets[j] - amount)

    def gap_rear_exists(self, k):
        """= VDC_GapRearExists: есть ли хоть один живой шар за стыком."""
        s = self.s
    ...
    def break_shot_stats(self):
        """Shot ended without destroying balls: no gap bonus, break chain/streak."""
        self.s.bullet_gap_count = 0
        self.s.stat_chain_count = 0

    def award_gap_bonus(self):
        """Successful shot destroyed balls after passing through a visible gap."""
        s = self.s
...
class App:
    ...
    def update_flying(self):
        ...
        for b in self.flying:
            ...
            b.x += b.dx
            b.y += b.dy
            # Off-screen -> miss: no destroy, no gap bonus.
            if b.x < -32 or b.x > SCR_W + 32 or b.y < -32 or b.y > SCR_H + 32:
                e.break_shot_stats()
                self.log.write(f'# MISS frame={e.s.frame} (no destroy, chain/gap streak reset)\n')
                continue
            ...
            for i in range(e.s.slots_len):
                ...
                if abs(ddx) < COLLISION_BBOX_HALF and abs(ddy) < COLLISION_BBOX_HALF:
                    ...
                    matched = e.insert_at(target_idx, b.color)
                    if matched:
                        e.award_gap_bonus()
    ...
    def render(self):
        ...
        jname = {0: 'none', 1: 'PULL', 2: 'CATCH-UP'}[s.gap_junction]
        info.append(f'GapJunction:  {s.gap_junction} ({jname})')
        info.append(f'GapPullVp:    {s.gap_pull_vp / 10:.1f} смп/кадр')
        info.append(f'GapTempo:     {s.gap_tempo / 10:.1f} px/кадр (+{s.gap_dec_acc}/10)')
        info.append(f'GapAccum:     {s.gap_accum}/{GAP_ACCUM_STEP}')
        if s.frame - s.last_merge_frame < 20:
            info.append('*CLICK* (слияние PULL)')
