...
def parse_level_records(level_num: int, level_name: str) -> list[dict[str, int]]:
    ...
    while i + 1 < len(lines):
        ...
        page, ram_lo, ram_hi, size = vals[:4]
...
def build_generated_mask(level_num: int, level_name: str, tint: tuple[int, int, int], alpha_scale: float) -> Image.Image:
    ...
    files = sorted((ROOT / "Graphics" / "levels" / "Converted" / "top_masks").glob(f"top_mask_l{level_num:02d}_{level_name}_*.bin"))
    if len(records) != len(files):
        raise RuntimeError(f"record/file mismatch for L{level_num:02d}: records={len(records)} files={len(files)}")
    mask = Image.new("RGBA", SCREEN, (0, 0, 0, 0))
    for rec, path in zip(records, files):
        a = l4_alpha(path.read_bytes(), rec["stride"], rec["height"])
        if alpha_scale != 1.0:
