...
.deactivate:        XOR  A
                    LD   (Bullet_Active), A
                    LD   A, (Bullet_TunnelSeen)
                    OR   A
                    JR   NZ, .expire_no_bonus
                    JP   VDC_AwardGapBonus            ; expire без hit → gap bonus / chain break
.expire_no_bonus:   XOR  A
                    LD   (Bullet_TunnelSeen), A
                    RET
...
Bullet_VY:          DEFB 0
Bullet_Color:       DEFB 0
Bullet_TunnelSeen:  DEFB 0                            ; bullet passed near an inTunnel ball; no gap bonus SFX on expire
