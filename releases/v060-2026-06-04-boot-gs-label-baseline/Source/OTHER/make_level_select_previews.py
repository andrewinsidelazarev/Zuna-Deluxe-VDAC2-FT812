...
def write_pages(stem: str, data: bytes) -> None:
    ...
    if pages != 6:
        raise ValueError(f"{stem}: expected 6 pages, got {pages}")
...
def build(stem: str, bg_name: str, pal_name: str) -> None:
    ...
    img = Image.fromarray(rgba, "RGBA").resize((PREVIEW_W, PREVIEW_H), Image.Resampling.LANCZOS)
    ...
    print(f"{stem}: {len(data)} bytes ({PREVIEW_W}x{PREVIEW_H} ARGB4), 6 pages")
