"""Generate <NN-name>-4-3.dat in every Graphics/levels/Original/<NN-name>/ folder.
...
make_track_640.py:
  ...
  - tangent precompute with window=±8, smoothed by wrap-aware moving avg
    width=5
  - output: word LE N + N×(X word, Y word, tangent byte, flags byte)
...
LEVELS_DIR = ROOT / 'Graphics' / 'levels' / 'Original'
...
TRACK_FLAG_OVERRIDES = {
    "13-groovefest-4-3.dat": [
        # Upper-right overpass: these balls are above the tunnel cover.
        (1614, 1770, TRACKF_DRAW_ABOVE),
    ],
}
...
def parse_dat(path: Path):
    ...
    for _ in range(curve_len):
        if o + 4 > len(data):
            break
        t1 = struct.unpack_from('<b', data, o + 0)[0]
        t2 = struct.unpack_from('<b', data, o + 1)[0]
        dx = struct.unpack_from('<b', data, o + 2)[0]
        dy = struct.unpack_from('<b', data, o + 3)[0]
        o += 4
        x += dx / 100.0
        y += dy / 100.0
        flags = (TRACKF_TUNNEL if t1 else 0) | (TRACKF_DRAW_ABOVE if t2 else 0)
        pts.append((x, y, flags))
    return pts
...
def build_track(dat_path: Path) -> tuple[bytes, int, int, int, int, int]:
    ...
    for k in range(n_out):
        ...
        flags_out.append(raw[src_i + 1][2])
    ...
    out = bytearray()
    out += struct.pack('<H', n)
    for (x, y), tan, flags in zip(points, tans, flags_out):
...
def apply_track_flag_overrides(name: str, payload: bytes) -> bytes:
    overrides = TRACK_FLAG_OVERRIDES.get(name)
    if not overrides:
        return payload
    out = bytearray(payload)
    n = struct.unpack_from('<H', out, 0)[0]
    for first, last, flags in overrides:
        if first < 0 or last >= n:
            raise ValueError(f"{name}: override {first}..{last} outside sample count {n}")
...
def main() -> int:
    ...
    for folder in folders:
        ...
        for dat in dats:
            ...
            payload = apply_track_flag_overrides(out_path.name, payload)
