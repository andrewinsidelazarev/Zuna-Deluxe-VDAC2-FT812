...
def audit_win_visual_ops(label: str, ops: list[DLOp], winexp_handle: int) -> bool:
    ok = True
    print(f"\n=== {label} ===")
    ...
    print(f"WINEXP handle={winexp_handle} indices: {win_indices[:8]}")

    if not win_indices:
        print(f"FAIL: no WINEXP BITMAP_HANDLE({winexp_handle}) found")
        ok = False
...
def main() -> int:
    ...
    ok = True
    winexp_handle = sym["Core.ZL_WINEXP_HANDLE"]
    ok &= audit_win_visual_ops(
        "DrawWinStateVisual only",
        emitted_ops(fs, "Core.DrawWinStateVisual"),
        winexp_handle,
    )
