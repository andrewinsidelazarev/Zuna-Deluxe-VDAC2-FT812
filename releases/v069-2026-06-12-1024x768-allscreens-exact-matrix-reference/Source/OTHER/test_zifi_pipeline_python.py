...
def main() -> int:
    ...
    # 1024×768 port: the PAK packer scales sample X/Y ×1.6 then page-splits
    # (make_level_pack.scale_track_samples + pagesplit_track). Compare against
    # exactly that canonical packed blob for L1 via read_track_blob(0).
    # (Converted/track_640.bin is a stale single-level artifact with an
    # inconsistent header and is no longer the packed source.)
    from make_level_pack import read_track_blob
    expected_tr = read_track_blob(0)
    if track_data[: len(expected_tr)] == expected_tr:
        print(f"  OK track stream matches packed L1 track ({len(expected_tr):,} bytes)")
    else:
