...
; ----------------------------------------------------------------------------
ZL_BALL_W       EQU 32                                ; native classic ball cell
ZL_BALL_H       EQU 32
ZL_BALL_HALF    EQU 16                                ; центр rect'а (= pivot для cmd_rotate)
ZL_CELL_SAMPLES EQU 32                                ; samples между шарами (= ball, плотные)
...
ZL_DrawFrame:
                ...
                ; --- VDC chain rendering: atlas cell spin frame without per-ball matrix.
                ; Vertex2f((X-16)*16, (Y-16)*16) → screen position top-left rect'а.
                ; Cell = color*16 + spin.
                LD   A, (VDC_SlotsLen)
                OR   A
                JP   Z, .ChainEnd
...
.ChainLoop:     PUSH BC                               ; сохранить count+i
                ...
                LD   BC, 16*16
                AND  A
                SBC  HL, BC                           ; HL = (X-16)*16
                LD   B, H : LD C, L                   ; BC = (X-16)*16
                CALL FT.Coprocessor.Vertex2f
.ChainSkip:     POP  BC
                INC  C
                DEC  B
