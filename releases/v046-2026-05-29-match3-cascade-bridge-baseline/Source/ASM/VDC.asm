...
VDC_ScanForNewMatch:
                ...
                OR   A
                RET  Z
                ; Пока в цепочке есть уже зафиксированные GAP/CASCADE markers,
                ; новая проверка match-3 преждевременна: она видит только часть
                ; визуально сомкнувшегося run и режет длинную группу по маркерам.
                ; VDC_DoGapStep удаляет markers выше по кадру; когда последний
                ; marker уйдёт, VDC_SetShot2OnNeighbors поставит Shot2 на реальную
                ; границу сомкнувшейся группы, и DetectMatch3 увидит весь run.
                PUSH AF
                PUSH BC
                PUSH HL
                LD   B, A
