#!/usr/bin/env python3
"""Pre-render the "LOADING LEVELS..." banner in nativealien48 (48 px, gradient).
...
single ARGB4 bitmap and blit it. Same nativealien48 source + red-yellow gradient
...
Output:
  ...
  Source/ASM/loading_text_meta.inc              — LOADING_TEXT_W / _H
"""
import subprocess
...
def main():
    ...
    # First pass: total width = sum of advances.
    total_w = 0
    for ch in TEXT:
        if ch == ' ':
            total_w += SPACE_WIDTH
        elif ch in char_index:
            total_w += widths[char_index[ch]]
        else:
            print(f'  WARNING: char {ch!r} missing in font'); total_w += SPACE_WIDTH
    atlas = np.zeros((TARGET_H, total_w, 4), dtype=np.uint8)

    # Second pass: blit each glyph rect at the advancing cursor.
    cx = 0
    for ch in TEXT:
        if ch == ' ':
            cx += SPACE_WIDTH
        ...
        cx += adv
    print(f'  "{TEXT}" -> {total_w}x{TARGET_H}')
    ...
    with open(OUT_INC, 'w', encoding='utf-8') as f:
        ...
        f.write(f'; Pre-rendered "{TEXT}" nativealien48 {TARGET_H}px ARGB4 banner.\n\n')
