...
DrawBlackTransitionFrame:
                ...
                ; Ждём INT_SWAP = чёрный кадр реально на экране. Флаг НЕ чистим —
                ; его съест wait_int следующего MenuSwapFrame (протокол ожидания).
.wait_black:    FT_RD_REG8 FT_REG_INT_FLAGS
                AND  FT_INT_SWAP
                JR   Z, .wait_black
                RET
