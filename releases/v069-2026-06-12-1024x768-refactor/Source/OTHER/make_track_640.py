...
Формат track_640.bin (6 байт на sample = X, Y, tangent angle, flags):
  word LE : N (количество семплов)
  N × 6 байт: X (sword), Y (sword), tangent (byte 0..255 = 0..360°),
              flags (bit0=tunnel/no bullet hit, bit1=draw above top layer)
...
При N=2762: 2 + 2762*6 = 16574 байт (длинные треки режутся на две страницы).
...
def parse_dat(path):
    ...
    for _ in range(curve_len):
        ...
        x += dx / 100.0
        y += dy / 100.0
        flags = (0x01 if t1 else 0) | (0x02 if t2 else 0)
...
out = bytearray()
out += struct.pack('<H', n)
for (x, y), tan, flags in zip(points, tangents, flags_out):
