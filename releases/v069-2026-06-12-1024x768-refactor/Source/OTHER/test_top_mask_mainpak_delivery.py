...
def main() -> int:
    ...
    failures: list[str] = []
    if not entries:
        failures.append("expected packed top-mask entries in main_pak_table, got none")
    pak = PAK.read_bytes()
    ...
    expected = list(range(0xCC, 0xCC + len(entries)))
    if pages != expected:
        failures.append(f"top-mask pages are {pages}, expected contiguous #{expected[0]:02X}..#{expected[-1]:02X}")
    if len(entries) > 52:
        failures.append(f"top-mask pages exceed #CC..#FF budget: {len(entries)}")
    if failures:
        print("FAIL: top-mask ZUMAMAIN.PAK delivery")
        for item in failures:
            print(f"- {item}")
        return 1
    print("PASS: packed top-mask pages are delivered byte-for-byte in ZUMAMAIN.PAK")
    return 0
