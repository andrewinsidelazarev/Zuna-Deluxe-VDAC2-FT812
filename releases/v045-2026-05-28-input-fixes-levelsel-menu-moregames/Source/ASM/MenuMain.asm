...
;   0 = Adventure, 1 = More, 2 = Quit (Gauntlet/Options без действия — пропущены).
...
.chk_down:      ; nav-вниз = Down
                ...
                LD   A, (MenuSelection)
                CP   2
                JR   NC, .highlight                        ; уже на нижней (Quit)
                INC  A
                LD   (MenuSelection), A
...
.not_adv:       CP   1
                JR   NZ, .not_more
                LD   A, 1 : LD (MenuMoreClick), A : RET
.not_more:      LD   A, 1 : LD (MenuQuitClick), A
                RET
...
.not0:          CP   1
                ...
                JR   .apply
.not1:          LD   HL, MenuButtonStateQuit
.apply:         LD   A, (HL)
                OR   A
...
MenuSelection:            DEFB 0   ; выбранная клавиатурой кнопка: 0=Adventure,1=More,2=Quit
