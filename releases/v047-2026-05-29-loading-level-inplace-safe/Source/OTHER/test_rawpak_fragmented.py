...
K_SEGMENTS = 8
GAP = 3                      # free clusters left between segments (forces a new run)
...
def fragment(img: Fat32Image) -> tuple[int, int, list[tuple[int, int]]]:
    ...
    # all of [start, start+n) is now free (the old chain was contiguous)

    # Reuse exactly that freed region, but place the K logical segments into
    # PERMUTED physical blocks so consecutive logical segments are never
    # physically adjacent -> every segment boundary becomes a new run, with no
    # extra free space required. perm = even block indices then odd ones, so the
    # block-index step between consecutive logical segments is never +1.
    seg_len = (n + K_SEGMENTS - 1) // K_SEGMENTS
    perm = list(range(0, K_SEGMENTS, 2)) + list(range(1, K_SEGMENTS, 2))
    segs: list[tuple[int, int]] = []
    remaining = n
    for j in range(K_SEGMENTS):
        if remaining <= 0:
            break
        L = min(seg_len, remaining)
        seg_start = start + perm[j] * seg_len       # physical block perm[j]
        segs.append((seg_start, L))
        remaining -= L

    # sanity: all target clusters must be free and inside the freed region
    for s, L in segs:
        for c in range(s, s + L):
            if not (start <= c < start + n) or img.get_fat(c) != 0:
                raise RuntimeError(f"target cluster {c} not free/in-region")

    # write content + link the chain across segments (in logical order)
    flat = [c for s, L in segs for c in range(s, s + L)]
