...
GPU note:
  ...
  Initial endpoint template generation uses OpenCL in GPU mode; candidate selection, seam-aware passes, residual diffusion and output writing remain CPU.
""".strip()
...
class OpenCLBlockEvaluator:
  ...
  def evaluate_errors(self, records: Sequence[tuple[int, int, int]], spec: FormatSpec,
                      weights: tuple[float, float, float]) -> list[float]:
    if not records:
      return []
    return self.evaluate_errors_arrays(
  ...
  def evaluate_records_arrays(self, block_indices_in: Sequence[int], c0s_in: Sequence[int], c1s_in: Sequence[int],
                              spec: FormatSpec, weights: tuple[float, float, float],
                              include_decoded: bool = True) -> list[tuple[int, Candidate]]:
    count = len(block_indices_in)
    if count == 0:
      return []
    ...
    # eval_pairs writes decoded RGB unconditionally, but for effort 0..5 the
    # GPU path does not need decoded pixels for intermediate candidates. Avoid
    # copying 48 bytes/candidate back to Python and fill decoded lazily only
    # for preview/seam/residual paths.
    ...
    for i in range(count):
      selector_bytes = selectors[i].tobytes()
      cand = Candidate(
        c0=int(c0s[i]),
        c1=int(c1s[i]),
        selectors=selector_bytes,
        # Keep DXT packed selectors lazy in GPU mode. Most intermediate
        # candidates are rejected before output, and packing 16 selectors for
        # every temporary Candidate showed up as Python overhead in --profile.
        packed_selectors=b"",
        decoded=decoded[i].tobytes() if include_decoded else b"",
        error=float(errors[i]),
      )
      out.append((int(block_indices[i]), cand))
    if self.profile is not None:
      self.profile.add_time("candidate.build", time.perf_counter() - build_t0)
      self.profile.add_count("candidate.objects.gpu", len(out))
    return out
  ...
  def evaluate_records(self, records: Sequence[tuple[int, int, int]], spec: FormatSpec,
                       weights: tuple[float, float, float], include_decoded: bool = True) -> list[tuple[int, Candidate]]:
    if not records:
      return []
    return self.evaluate_records_arrays(
      [r[0] for r in records],
      [r[1] for r in records],
      [r[2] for r in records],
      spec,
...
def add_candidate_limited(candidates: dict[tuple[int, int, bytes], Candidate], cand: Candidate, limit: int) -> None:
  # GPU path helper: fixed-size retained insertion. Avoid repeated full
  # dict->list sorting in hot merge paths; the pool is tiny, but this function
  # is called hundreds of thousands of times on large images.
  if limit <= 0:
    return
  ...
  worst_key: tuple[int, int, bytes] | None = None
  worst_error = -1.0
  for item_key, item in candidates.items():
    if item.error > worst_error:
      worst_error = item.error
      worst_key = item_key

  if worst_key is not None and cand.error + 1e-9 < worst_error:
...
def build_initial_pair_cache(blocks: Sequence[bytes], effort: int, weights: tuple[float, float, float],
                             profile: ProfileMetrics | None = None) -> list[list[tuple[int, int]]]:
  t0 = time.perf_counter()
  cache: dict[bytes, list[tuple[int, int]]] = {}
  out: list[list[tuple[int, int]]] = []
  generated_pairs = 0
  for block in blocks:
    pairs_565 = cache.get(block)
    if pairs_565 is None:
      pairs_565 = initial_endpoint_pairs_565(block, effort, weights)
      cache[block] = pairs_565
      if profile is not None:
        profile.add_count("gpu.initial_template.unique_blocks", 1)
    else:
      if profile is not None:
        profile.add_count("gpu.initial_template.cache_hits", 1)
    generated_pairs += len(pairs_565)
    out.append(pairs_565)
  if profile is not None:
...
def endpoint_variation_pairs(cand: Candidate, effort: int, radius: int, max_extra: int) -> list[tuple[int, int]]:
  if radius <= 0 or max_extra <= 0:
    return []
  ...
  def try_parts(parts: list[int]) -> bool:
    if len(pairs) >= max_extra:
      return False
    pairs.append((join_565(parts[:3]), join_565(parts[3:])))
    return len(pairs) < max_extra
...
def block_error_against_adjusted_target_fast(decoded: bytes, original: bytes, adjustment: Sequence[float],
                                             weights: tuple[float, float, float]) -> float:
  total = 0.0
  for i in range(48):
    ch = i % 3
    target = clamp_float(float(original[i]) + adjustment[i], 0.0, 255.0)
    d = float(decoded[i]) - target
    total += weights[ch] * d * d
  return total
...
def edge_error(orig_a: bytes, dec_a: bytes, orig_b: bytes, dec_b: bytes, direction: str,
               weights: tuple[float, float, float]) -> float:
  total = 0.0
  if direction == "right":
    pairs = [((y * 4 + 3), (y * 4 + 0)) for y in range(4)]
  elif direction == "left":
    pairs = [((y * 4 + 0), (y * 4 + 3)) for y in range(4)]
  elif direction == "down":
    pairs = [((3 * 4 + x), (0 * 4 + x)) for x in range(4)]
  elif direction == "up":
    pairs = [((0 * 4 + x), (3 * 4 + x)) for x in range(4)]
  else:
    raise ValueError(direction)

  for ia, ib in pairs:
    oa = orig_a[ia * 3:ia * 3 + 3]
    ob = orig_b[ib * 3:ib * 3 + 3]
    da = dec_a[ia * 3:ia * 3 + 3]
    db = dec_b[ib * 3:ib * 3 + 3]
    for ch in range(3):
      orig_delta = int(oa[ch]) - int(ob[ch])
      dec_delta = int(da[ch]) - int(db[ch])
      d = dec_delta - orig_delta
      total += weights[ch] * d * d
  return total


def neighbor_indices(index: int, blocks_x: int, blocks_y: int) -> list[tuple[int, str]]:
  x = index % blocks_x
  y = index // blocks_x
  out: list[tuple[int, str]] = []
  if x > 0:
    out.append((index - 1, "left"))
  if x + 1 < blocks_x:
    out.append((index + 1, "right"))
  if y > 0:
    out.append((index - blocks_x, "up"))
  if y + 1 < blocks_y:
    out.append((index + blocks_x, "down"))
  return out
...
def seam_score(index: int, cand: Candidate, selected: Sequence[Candidate], original_blocks: Sequence[bytes],
               blocks_x: int, blocks_y: int, weights: tuple[float, float, float]) -> float:
  score = 0.0
  for nidx, direction in neighbor_indices(index, blocks_x, blocks_y):
    score += edge_error(
      original_blocks[index], cand.decoded,
      original_blocks[nidx], selected[nidx].decoded,
      direction,
      weights,
    )
  return score
...
def block_error_against_adjusted_target(cand: Candidate, original: bytes, adjustment: Sequence[float],
                                        weights: tuple[float, float, float]) -> float:
  total = 0.0
  for i in range(48):
    ch = i % 3
    target = clamp_float(float(original[i]) + adjustment[i], 0.0, 255.0)
    d = float(cand.decoded[i]) - target
    total += weights[ch] * d * d
  return total


def diffuse_residual(selected: list[Candidate], candidate_lists: Sequence[list[Candidate]], original_blocks: Sequence[bytes],
                     blocks_x: int, blocks_y: int, cfg: EffortConfig,
                     weights: tuple[float, float, float]) -> None:
  # Simplified residual diffusion: residual is stored as a block-local RGB bias.
  # This intentionally stays weak and clamped so it does not smear real hard edges.
  residual = [[0.0] * 48 for _ in range(len(selected))]

  def add_avg_residual(dst: int, avg: tuple[float, float, float], factor: float) -> None:
    for pix in range(16):
      base = pix * 3
      for ch in range(3):
        v = residual[dst][base + ch] + avg[ch] * factor
        residual[dst][base + ch] = clamp_float(v, -cfg.residual_clamp, cfg.residual_clamp)
  ...
  for y in range(blocks_y):
    for x in range(blocks_x):
      ...
      for cand in candidate_lists[idx]:
        score = block_error_against_adjusted_target(cand, original_blocks[idx], residual[idx], weights)
        score += cfg.seam_weight * seam_score(idx, cand, selected, original_blocks, blocks_x, blocks_y, weights)
        if score < best_score:
          best_score = score
          best = cand
      selected[idx] = best
      ...
      for pix in range(16):
        base = pix * 3
        for ch in range(3):
          avg[ch] += float(original_blocks[idx][base + ch]) - float(best.decoded[base + ch])
      avg_tuple = tuple((avg[ch] / 16.0) * cfg.residual_strength for ch in range(3))
...
def gpu_score_records_chunked(evaluator: OpenCLBlockEvaluator, records: Sequence[tuple[int, int, int]],
                              spec: FormatSpec, weights: tuple[float, float, float],
  ...
  for start in range(0, len(records), batch_limit):
    out.extend(evaluator.evaluate_errors(records[start:start + batch_limit], spec, weights))
  return out


def gpu_evaluate_records_chunked(evaluator: OpenCLBlockEvaluator, records: Sequence[tuple[int, int, int]],
                                 spec: FormatSpec, weights: tuple[float, float, float],
                                 batch_limit: int = 262144, include_decoded: bool = True) -> list[tuple[int, Candidate]]:
  if not records:
    return []

  out: list[tuple[int, Candidate]] = []
  for start in range(0, len(records), batch_limit):
    out.extend(evaluator.evaluate_records(records[start:start + batch_limit], spec, weights, include_decoded))
  return out
...
def gpu_evaluate_batch_chunked(evaluator: OpenCLBlockEvaluator, batch: RecordBatch,
                               spec: FormatSpec, weights: tuple[float, float, float],
                               batch_limit: int, include_decoded: bool = True) -> list[tuple[int, Candidate]]:
  if len(batch) == 0:
    return []
  ...
  for start in range(0, len(batch), batch_limit):
    ...
    out.extend(evaluator.evaluate_records_arrays(
      batch.block_indices[start:end],
      batch.c0s[start:end],
      batch.c1s[start:end],
      spec,
...
def finish_block_candidates_gpu(blocks: Sequence[bytes], spec: FormatSpec, effort: int,
  ...
  for pass_idx in range(cfg.refine_iters):
    ...
    if not active_seen:
      for i in range(len(work_items.active)):
        work_items.active[i] = False
...
def pack_l2_block_selectors(selectors: bytes) -> int:
  v = 0
  for i, sel in enumerate(selectors):
    v |= (int(sel) & 0x03) << (i * 2)
  return v
...
def pack_l4_block_selectors(selectors: bytes) -> bytes:
  out = bytearray(8)
  for i in range(0, 16, 2):
    out[i // 2] = ((selectors[i] & 0x0F) << 4) | (selectors[i + 1] & 0x0F)
  return bytes(out)
...
def write_outputs(encoded: EncodedImage, output_base: str, out_kinds: set[str], split: bool,
                  preview: bool, zlib_level: int, progress: Progress | None = None) -> list[Path]:
  ...
  if "dxp" in out_kinds:
    ...
    write_file(dxp_path, make_dxp(encoded, raw, zlib_level))
...
def run(args: argparse.Namespace) -> int:
  ...
  for spec_key in ("l2", "l4"):
    ...
    output_files.extend(write_outputs(encoded, output_base, out_kinds, args.split, args.preview, args.zlib_level, progress))
