...
FadeLevelSelectToGameplay:
                ...
                SetPage3 #04                            ; slot 3 -> gameplay overlay BEFORE asset load
                                                         ; (LoadGameplayAssets runs VDC_Init/Frog_Init/Bullet_Init on #04)
                LD   A, #04 : LD (CurrentCodePage), A    ; track scene page (shared decompressors restore to it)
                CALL Core.ShowCurrentLevelLoadingScreen
...
; LOADING_TEXT живёт в той же области, что frame strips, поэтому перед кадром
; заново заливаем его в RAM_G, рисуем один стабильный чёрный loading-кадр,
; затем запускаем блокирующую загрузку следующего уровня.
