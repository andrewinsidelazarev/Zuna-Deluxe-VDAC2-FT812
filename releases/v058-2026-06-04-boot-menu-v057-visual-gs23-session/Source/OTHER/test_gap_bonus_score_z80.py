...
def main() -> int:
    ...
    exp = (500, 500, 1)
    print(f"first gap dist=0: score={got[0]} gauge={got[1]} count={got[2]} expected={exp}")
    if got != exp:
    ...
    got = (score24(), word(sym["Core.VDC_GaugeScore"]), emu.mem.read(sym["Core.VDC_BulletGapCount"]))
    exp = (1500, 1500, 2)
    print(f"second consecutive dist=0: score={got[0]} gauge={got[1]} count={got[2]} expected={exp}")
    if got != exp:
