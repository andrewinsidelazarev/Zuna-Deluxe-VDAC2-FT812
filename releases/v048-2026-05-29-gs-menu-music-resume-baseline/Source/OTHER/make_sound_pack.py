...
22050 Hz; SND_CHANT1 is packed at 8000 Hz for the current GS playback path.
...
def resample_u8_mono(pcm: bytes, src_rate: int, dst_rate: int) -> bytes:
    if src_rate == dst_rate:
        return pcm
    if not pcm:
        return pcm
    out_len = max(1, round(len(pcm) * dst_rate / src_rate))
    ...
    for i in range(out_len):
        ...
        if left >= len(pcm) - 1:
            ...
            continue
        frac = pos - left
        sample = pcm[left] + (pcm[left + 1] - pcm[left]) * frac
        out[i] = max(0, min(255, round(sample)))
    return bytes(out)
...
def main() -> int:
    ...
    for sound_id, (name, filename) in enumerate(SOUNDS):
        ...
        target_rate = RATE_OVERRIDES.get(name, rate)
        if target_rate != rate:
            pcm = resample_u8_mono(pcm, rate, target_rate)
            rate = target_rate
        if name in TRIM_SILENCE_TAIL:
            pcm = trim_u8_silence_tail(pcm, TRIM_SILENCE_TAIL[name])
        if name in FADE_TAIL_SAMPLES:
            pcm = fade_u8_tail_to_silence(pcm, FADE_TAIL_SAMPLES[name])
    ...
    lines = [
        ...
        "; ZUMASND.PAK stores raw 8-bit mono 22050 Hz GS sound effects.",
