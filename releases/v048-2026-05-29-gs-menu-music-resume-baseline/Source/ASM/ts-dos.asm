...
.havePart:     LD   A, (IX + 8)  : LD (RawPak_PartLba + 0), A   ; partition start LBA
                ...
                ; WC's block-addressing flag (#6015) is not mapped in the SPG, so
                ; probe: try block addressing (real SDHC); if the BPB at part_lba is
                ; invalid, fall back to byte addressing.
                LD   A, 1
                LD   (sd_blkt), A
...
.nextComp:      LD   A, (HL)
                ...
                POP  HL
                JR   .nextComp
.findPakHere:   CALL RawPak_CopyPakTarget
                XOR  A
                LD   (RawPak_FindWantDir), A
                LD   A, 1
                LD   (RawPak_CheckSize), A
...
ZiFi_AudioPakOpen:
                CALL RawPak_OpenRoot             ; parse BPB/FAT and keep the normal level-pack cache warm
                RET  NC
