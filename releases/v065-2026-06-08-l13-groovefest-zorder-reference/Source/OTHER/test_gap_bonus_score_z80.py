...
def main() -> int:
    ...
    def set_gap(dist: int, count: int, chain: int = 7) -> None:
        emu.set_byte(sym["Core.VDC_BulletGapMinDist"], dist)
        emu.set_byte(sym["Core.VDC_BulletGapCount"], count)
        emu.set_byte(sym["Core.VDC_StatChainCount"], chain)
    ...
    got = (
        ...
        emu.mem.read(sym["Core.VDC_StatChainCount"]),
    )
    exp = (500, 500, 1 if 500 >= target else 0, 1, 7)
    print(f"first gap dist=0: score={got[0]} gauge={got[1]} full={got[2]} count={got[3]} chain={got[4]} expected={exp}")
    if got != exp:
    ...
    exp = (1500, 1500, 1 if 1500 >= target else 0, 2, 7)
    print(f"second consecutive dist=0: score={got[0]} gauge={got[1]} full={got[2]} count={got[3]} chain={got[4]} expected={exp}")
    if got != exp:
    ...
    exp = (1500, 1500, 0, 7)
    print(f"matched non-gap dist=25: score={got[0]} gauge={got[1]} count={got[2]} chain={got[3]} expected={exp}")
    if got != exp:
