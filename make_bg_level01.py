...
with open(os.path.join(HERE, 'bg_level01.bin'), 'wb') as f:
    f.write(out)
...
for i in range(n_pages):
    ...
    with open(os.path.join(HERE, f'bg_level01_p{i:02d}.bin'), 'wb') as f:
        f.write(chunk)

print(f'level {level}: {os.path.basename(src)}')
print(f'wrote bg_level01.bin: {len(out)} bytes ({W}x{H} RGB565), {n_pages} pages')
im.save(os.path.join(HERE, '_bg_level01_preview.png'))
