...
; ============================================================================
; Init_Video — инициализация видеорежима 1024×768 через VDAC2 / FT812
; ----------------------------------------------------------------------------
...
;
; Выход:
;   A=0, Z=1   — успех (1024×768 включён через FT812)
...
Init_Video:     ; --- 1. Sanity-check: VDAC2 присутствует на плате? --------
                ...
                ; --- 4. Переключить таймминги на 1024×768 @ 59 Hz --------
                ; VM_1024_768_59Hz: PCLK 64 МГц (F_MUL=8), H 24/136/160/1024,
                ;                   V 3/6/29/768 → HCYCLE 1344, VCYCLE 806.
                ; Это ровно ×1.6 от 640×480 (8:5) — чистый NEAREST-апскейл всех
                ; слоёв. Per-line бюджет растёт 832→1344 пропорционально ширине,
                ; число строк 480→768 — общий fill-бюджет масштабируется ~×2.56.
