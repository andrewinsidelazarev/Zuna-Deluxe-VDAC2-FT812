...
def prepare_host_package(root: Path) -> list[Path]:
    ...
    files = [
        ...
        (root / "spgbld_vdac2.ini", package_dir / "SPGBLD.INI"),
    ...
    (package_dir / "README.TXT").write_text(
        ...
        "Run ZUMA_VD2.SPG. Level pack is ZUMALVL.PAK in this folder.\n",
        encoding="ascii",
    )
...
def main() -> int:
    ...
    ap.add_argument("--base-img", default=r"C:\Users\Администратор\Desktop\unreal_x64\wc.img")
    ap.add_argument("--out-img", default=r"Build\wc_zuma_vdac2.img")
    args = ap.parse_args()

    root = Path.cwd()
    base_img = Path(args.base_img)
    out_arg = Path(args.out_img)
    ...
    in_place = base_norm == out_norm
    if not in_place:
        shutil.copy2(base_img, out_img)
    ...
    print(f"image: {out_img}{' (in-place)' if in_place else ''}")
