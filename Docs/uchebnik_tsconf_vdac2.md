...
Кейс из порта HMM2 полезен как проверка переносимости VDAC2-инициализации. На той же плате уже работали Zuma, TSafari и Evogram, поэтому монитор, сам VDAC2 и режим 640×480 @ 74 Hz не были первыми подозреваемыми. Симптом был такой: программа стартует, обычный TS-Config экран может появляться, но при переходе на FT812 монитор пишет «нет сигнала» или во время старта виден старый мусорный TS-Config framebuffer.

Что было проверено и отброшено:

- **RAM_G и бюджет кадра** — не причина. Upload terrain/object atlas проходил byte-for-byte, RAM_G не выходил за `#100000`, Display List укладывался в RAM_DL.
- **Игровой процесс и render loop** — не причина для «нет сигнала». Canary, который останавливался сразу после `Init_Video`, воспроизводил проблему до `Game_Init`/`Render_Frame`.
- **Монитор/частота** — не причина. Та же железка стабильно держит рабочие VDAC2-программы.
- **Старое состояние TS-Conf** — проверялось F12/reset; проблема была в boot-пути конкретной программы, а не в «залипшем» видеорежиме.

Фактические проблемы оказались в деталях стартовой последовательности:

1. В HMM2 была изменена реализация `FT_DELAY`: вместо задержки через `HALT` стоял короткий busy-loop. Для `FT_BOOT_UP` это критично: host commands `PWRDOWN_ -> CLKEXT -> CLKSEL -> ACTIVE` требуют реальных пауз между шагами. В рабочих проектах задержка синхронизирована с frame interrupt:
...
2. Перед `FT_BOOT_UP` нужен один TS-Conf frame interrupt, как в Zuma. Это даёт железу время стабилизировать timing перед стартом FT812:
...
3. До переключения на `VID_FT812 | VID_NOGFX` монитор ещё показывает обычный TS-Config framebuffer. Если там осталась шахматка/мусор от диагностики, пользователь видит её во время `FT_BOOT_UP`. Поэтому перед ожиданием и FT812 boot полезно погасить TS-Config gfx:
...
Platform_Init:
                ...
                ; Пока FT812 просыпается, обычный TS-Conf экран не должен
                ; показывать старый framebuffer.
...
4. В `Init_Video` сам чёрный экран должен быть настоящим FT812 Display List, как в Zuma:
...
.EmptyDL:       FT_CLEAR_COLOR_RGB 0, 0, 0
                FT_CLEAR 1, 1, 1
                FT_DISPLAY
.EmptyDL_Size   EQU $ - .EmptyDL
```

Итоговая рабочая схема для HMM2:

- `FT_DELAY` вернуть к варианту с `HALT`, как в Zuma/TSafari/Evogram.
- Перед `Init_Video` выполнить `Init_Int` и дождаться первого frame interrupt.
- Перед `FT_BOOT_UP` погасить старый TS-Config framebuffer через `VID_NOGFX` и чёрный border.
- После `Init_Video` отключить TS-Conf frame interrupt (`DI`, `INT_Setting 0`), чтобы дальше синхронизация шла уже через FT812.
- Не лечить «нет сигнала» изменением `Render_Frame`: если canary падает до `Game_Init`, рендер не участвует.
- После восстановления сигнала вернуть игровой цикл и откатить экспериментальные FIFO/CMD-рендеры к проверенному direct `FT.WriteDL`, если они не нужны для конкретной задачи.
...
> (160/256 = `#xx0000A0`); (2) **точность важна не только по форме, но и по
...
Главное правило эпохи: **`CMD_SCALE` (`#FFFFFF28`) с нецелым множителем
использовать ЗАПРЕЩЕНО.** Причина — копроцессор FT812 не хранит масштаб как есть;
он вычисляет и хранит **обратную** `BITMAP_TRANSFORM`-матрицу с усечением до
формата 8.8. Для ×1.6 обратный коэффициент 1/1.6 = 0.625 = 160/256; но
копроцессор через своё деление выдаёт 159/256 ≈ 0.6211, что соответствует
эффективному масштабу 256/159 = 1.6101 — и спрайт «уезжает» тем сильнее, чем
дальше пиксель от точки трансформации, накапливая дрейф до +6 px по краю окна.
Это давало системный сдвиг (например, зелень под перекрашенной кнопкой MENU и
«чёрную полосу неба» лечили в т.ч. отказом от `CMD_SCALE`).

**Решение — запечённые `BITMAP_TRANSFORM`-слова напрямую в RAM_CMD/DL**, минуя
деление копроцессора. Для апскейла ×1.6 нужен обратный коэффициент 160/256:

```
                DEFD #150000A0   ; BITMAP_TRANSFORM_A = 160/256 (= 1/1.6, точно)
                DEFD #16000000   ; B = 0
                DEFD #17000000   ; C = 0
                DEFD #18000000   ; D = 0
...
Соблюсти «только запечённые матрицы» по форме мало — нужна и **точная
арифметика** при их вычислении в рантайме. Лягушка строит матрицу вращения
вокруг центра как LUT (см. главу 15.6 про инверсию), и коэффициент `A=E=cos·(1/1.6)`
поначалу считался шорткатом `cos·1.25 = cos + (cos asr 2)`. Для `cos=127` это
даёт 158, а не канонные 160 → эффективный масштаб 256/158 = **1.6203** вместо
1.6 → спрайт 122 px рисовался на 197.7 px при окне отрисовки 195 px → **тарелка
лягушки обрезана справа и снизу** на ~2.7 px.

Фикс — точное умножение `cos·323/256` (323 = 256 + 67, при cos=127 даёт ровно
160), через сложение без таблиц:

```
; Frog_Mul323Sh8 — In A=signed(-127..127), Out HL = A·323/256 (точно)
Frog_Mul323Sh8:   LD   C, A             ; сохранить знак
                  BIT  7, A : JR Z, .abs
                  NEG
.abs:             LD   D, A : LD E, 67
                  CALL Frog_Mul8x8u      ; HL = |A|·67
                  ... A = |A| + (|A|·67 >> 8)  ; = |A|·323/256, max 160
                  ... затем восстановить знак
```

Проверка по всем углам: при `cos=127` — ровно 160. Урок (универсальный,
даже вне FT812): **канонические константы прогонять через точную арифметику, а не
через удобный шорткат** — погрешность в 1/256 на масштабе вылезает обрезанным
краем спрайта.

**Субпиксельный довесок: округление draw-окна вниз тоже режет край.** Даже с
точной матрицей 160/256 остаётся вторая, независимая причина обрезки тарелки
справа/снизу: размер окна отрисовки `FT_BitmapSize` целочисленный, а
`FROG_SPR_DRAW = round(122·8/5) = round(195.2) = 195`. Истинный размер 195.2 px,
окно — 195 px → **0.2 px правого/нижнего края тарелки не помещаются в окно
`BITMAP_SIZE` и срезаются**, независимо от точности матрицы. Подложка-тарелка как
раз касается границы своего bbox, поэтому субпиксель виден именно на ней.
Решение: для спрайтов, чей значимый край лежит у границы draw-окна, размер
окна **округлять ВВЕРХ (`ceil`, 196) или давать +1 px запаса** — лишний прозрачный
пиксель по краю безопаснее срезанного значимого. Правило: при нецелом апскейле
**окно отрисовки = `ceil(W·8/5)`, не `round`** (см. также §12.3 BITMAP_SIZE при
upscale — там бар уже считается как `(W·8+4)/5` = ceil-форма).

### 35.5. Грабля №3: 15-битные `VERTEX2F` и 9-битные `VERTEX2II`

При ×1.6 экранные координаты раздуваются, и легко выйти за разрядность вершинных
команд FT812:

- **`VERTEX2F`** (формат 4, 1/16 px) — знаковое 15-битное поле, максимум +16383 =
  **1023.94 px**. Любая координата X/Y, превышающая ~1023 px после ×1.6,
  переполняется. На 1024×768 это буквально правый/нижний край экрана.
- **`VERTEX2II`** — беззнаковые 9-битные X/Y, максимум **511**. На 1024×768
  половина экрана уже недостижима этой командой.

Практическое следствие: элементы, чьи координаты после ×1.6 превышают пределы
(например, прогресс-бар на `Y = 356·1.6 = 569 > 511`), **нельзя** рисовать через
`VERTEX2II` — вместо этого `Cell + Vertex2f` с координатами в формате 4 (см.
`loader_resident.asm:DrawLoadingScreen`, комментарий «Vertex2ii не годится: Y…>511»).
Граната/гранулы и любые крупные координаты держать в `VERTEX2F` и не давать им
перевалить 1023 px.

### 35.6. Грабля №4: `BILINEAR + BORDER` — чёрная вертикальная линия на стыке при скролле неба

Конкретный симптом: при апскейле небо **скроллится** (две копии тайла едут встык,
шов перемещается по экрану), и на этом шве видна **движущаяся чёрная вертикальная
линия**. Причина: при `BILINEAR` фильтре и `BORDER`-режиме крайние ~1.5 px окна
битмапа становятся полупрозрачными (билинейная интерполяция на краю подмешивает
«ничто» = чёрное). Пока тайл статичен, шов можно «спрятать», но при скролле он
постоянно на виду — отсюда заметная вертикальная полоса, ползущая вместе с небом.
Её **не убрать ни оверлапом копий, ни подложкой** (обе попытки провалились —
«всё равно чёрная полоска»); полупрозрачный край есть всегда, чем бы стык ни
прикрывали.

Решение пользователя: **возвращай `NEAREST`** — у `NEAREST` край глухой, стык
плотный. Альтернатива (если нужен билинейный фон) — `REPEAT`-режим, но он требует
ширины тайла, кратной степени двойки. Для неба меню/level-select выбран `NEAREST`
встык (решение «консистентно для всех экранов»). Правило: **`BILINEAR+BORDER`
несовместим со стыковкой тайлов; для бесшовного тайлинга — `NEAREST` встык или
`REPEAT` со степенью двойки.**

### 35.7. Скейл ассетов и треков

Геометрия треков уровней пересчитана множителем **8/5** в координатах
(`make_track_*`/scale ×8/5), фон — апскейл `NEAREST` без перегенерации исходников.
Это держит «апскейл, не HD»: ассеты те же, движок рисует крупнее. Из-за ×8/5 в
координатах часть юнит-тестов (например, сверка вторых треков с исходником)
ложно «не сходится», если тест не учитывает скейл — это устаревший тест, а не
битый пак (см. также главу 36 про сборку).

### 35.8. Грабля №5: нецелый `NEAREST ×1.6` рвёт тонкие детали (шрифты, логотип ZX Evolution)

`NEAREST` при **нецелом** масштабе дублирует исходные пиксели **неравномерно**:
×1.6 = 8/5 означает «на 5 исходных пикселей → 8 экранных», то есть часть исходных
пикселей растягивается в 2 экранных, а часть — в 1. Для крупного фона это
незаметно, но **тонкие детали (1-px линии шрифтов, обводки мелких спрайтов)
рвутся**: одна и та же линия местами выходит 1 px, местами 2 px, с разрывами и
неровной толщиной. Именно так «рвался» спрайт логотипа **ZX Evolution** в
загрузчике при ×1.6.

Решения:

- **Логотип ZX Evolution** масштабируется честным **целым `NEAREST ×2`**
  (`BootEmitScale2xTransforms`, A=E=128/256), а не ×1.6: целое дублирование
...
дублирование), либо нативный/более крупный исходный кегль. Нецелый `NEAREST ×1.6`
оставить для крупного фона и больших спрайтов, где неравномерность пикселей не
бросается в глаза.

### Ловушки (свод)

- `CMD_SCALE` нецелый → дрейф из-за инверсии с усечением. Только запечённые
  `BITMAP_TRANSFORM` 160/256.
- Шорткат вместо точной арифметики константы → +1/256 → обрезанный спрайт.
- `VERTEX2F` > 1023 px и `VERTEX2II` > 511 переполняются на 1024×768.
- `BILINEAR+BORDER` → полупрозрачный край → чёрная вертикальная линия на стыке
  тайлов при скролле неба. `NEAREST` встык / `REPEAT` (степень двойки).
...
> **TL;DR.** Игра поставляется четырьмя `.PAK`-файлами на SD-карте: `ZUMAMAIN.PAK`
> (код + ассеты по 16К-страницам), `ZUMALVL.PAK` (22 уровня), `ZUMASND.PAK` (SFX
> для General Sound), `ZUMAAUD.PAK` (MOD-музыка). Глава 30 описала *рантайм-чтение*
> (FAT32/RawPak/CMD17). Здесь — *сборка* паков и **главная грабля цикла**:
> `make_main_pack.py --table` вшивает в `.asm` таблицу страниц, посчитанную по
> **бинарям прошлой сборки**, а `--pack` пакует **свежие**. При изменении размера
> кода первая сборка даёт пак, рассинхронизированный с вшитой таблицей →
> мусорный `lba` на последней стадии загрузки, обрезанный трек уровня. Лечение —
> **двойной проход** `table → sjasmplus → pack` до фикспойнта; вшито в
> `build_wc_img.cmd`. Это была ложная «регрессия подтяжки»: нули сжимаются `zx7`,
> реальный код — нет, блок вырос на сектор.

Глава 30 рассматривала вопрос «как Z80 *читает* PAK с карты»: прямой `CMD17`,
LFN-обход пути, FAT-цепочка, допущение непрерывности `LBA = PakLba + N`,
двухфазная загрузка SD→FT812. Эта глава — про *обратную* сторону того же конвейера:
**как PAK-файлы собираются на хосте, что в них лежит и почему их раскладка обязана
быть синхронна с вшитыми в прошивку таблицами**. Если глава 30 — про ридер, то
эта — про райтер и про самую дорогую грабли всего pipeline.

### 36.1. Четыре пака проекта: кто что грузит

Игра выносит на SD четыре независимых пак-файла. Их кладёт в каталог
`/Games/Zuma Deluxe VDAC2/` инжектор (`inject_zuma_to_wc_img.py`,
`prepare_host_package`):

| Файл | Собирает | Что внутри | Кто грузит в рантайме |
|---|---|---|---|
| `ZUMAMAIN.PAK` | `make_main_pack.py --pack` | Код-оверлеи + графика, разложенные по 16К-страницам TS-Conf | Загрузчик `ts-dos.asm` сразу после boot-SPG, по вшитой `MainPakPageTable` |
| `ZUMALVL.PAK` | `make_level_pack.py` | 22 уровня: bg(8 стр) / palette / track(+2-я стр) / title / preview | `RawPak_*` при выборе/смене уровня (глава 30) |
| `ZUMASND.PAK` | `make_sound_pack.py` | SFX — raw 8-бит моно 11025 Гц PCM, формат `ZSPD` | `GS_LoadSfxPackWithTable` в General Sound по `GS_SfxPreloadTable` |
| `ZUMAAUD.PAK` | `make_audio_pack.py` | Сырой MOD-файл меню (`zuma_gs4_1mb_gssafe.mod`) | Стриминг в GS на boot (глава 33) |

Принципиальная развилка: **`ZUMAMAIN.PAK` — это не файловые ассеты, а образ
страничной памяти**. Boot-SPG минимален (несколько страниц), а всё остальное —
207 страниц по 32 сектора (`MAIN_PAK_PAGE_COUNT EQU 207`,
`MAIN_PAK_SECTORS_PER_PAGE EQU 32` в `main_pak_table.inc`) — приезжает паком и
раскладывается строго постранично через `SetPage2_A`. Остальные три пака —
честные файлы с собственными TOC, которые читаются по требованию.

> Загрузка `ZUMAMAIN` (`ts-dos.asm`, `.pageLoop` ≈ строки 1592–1632): на каждую
> запись таблицы — `LD A,(HL)` страница → `SetPage2_A`, `IX=#8000`,
> `B=MAIN_PAK_SECTORS_PER_PAGE` секторов через `RawPak_ReadOneLogicalIX`, и
> `BootLoadingTick` каждые 8 секторов (`AND 7`) для прогресс-бара. Логический
> сектор пака — `(RawPak_LogCur)`, инкрементируется на 32 на страницу.

### 36.2. Как `make_main_pack.py` строит и таблицу, и пак из одного источника

Ключ к понимании грабли — то, что **таблица страниц и сам пак генерятся из одного
описания** `spgbld_vdac2.ini` (188 строк `Block=`), но в **разные моменты** и из
**разных версий бинарей**.

`parse_blocks()` читает каждую строку вида `Block = addr, page, файл`:

```
Block = #C000, #04, Build/main1_play.bin        ; addr!=0 → код-оверлей в page #04
Block = #0000, #07, Graphics/.../bg_paletted_p00.bin  ; addr==0 → данные, чанками по 16К
```

Логика разбора (`BLOCK_RE`, `parse_blocks`):

- `addr != 0` — это **код-оверлей** (например `main1_play.bin` грузится по `#C000`
  внутри своей 16К-страницы). Один блок = одна страница; размер файла **не влияет**
  на число записей (важно для §36.3).
- `addr == 0` — это **данные**: файл может занимать несколько страниц, число чанков
  = `ceil(size / 0x4000)`, каждый чанк → своя последовательная страница
  (`page + chunk`).
- **Boot-страницы пропускаются.** `BOOT_PAGES = {0x00, 0x05, 0x25, 0x26, 0x27,
  0x40, 0x41}` уже лежат в маленьком boot-SPG и переживают загрузку пака — их в
  пак не кладут (иначе бы перетёрли live-страницы загрузчика самим загрузчиком).

Дальше — две функции, между которыми и зарыта грабля:

```python
def write_table(blocks):       # --table
    # пишет Source/ASM/main_pak_table.inc:
    #   MAIN_PAK_PAGE_COUNT EQU <len(blocks)>
    #   MainPakPageTable: DEFB #page / DEFW sector ...
    sector = 0
    for page, _addr, rel, chunk in blocks:
        lines.append(f"DEFB #{page:02X}")
        lines.append(f"DEFW {sector}")
        sector += SECTORS_PER_PAGE   # +32 каждая страница

def write_pack(blocks):        # --pack
    blob = bytearray()
    for page, addr, rel, chunk in blocks:
        blob += read_page(ROOT/rel, addr, chunk)   # 16К на запись
    OUT_PAK.write_bytes(blob)
```

`read_page` для код-оверлея кладёт байты по их адресу внутри страницы
(`off = addr & 0x3FFF`), добивает нулями до полных 16К; для данных — нарезает
нужный чанк и тоже добивает до 16К. **Каждая запись таблицы строго соответствует
секторному окну `sector .. sector+31` в паке** — но только если набор и порядок
блоков идентичен между `--table` и `--pack`.

### 36.3. Главная грабля: рассинхрон table↔pack при росте кода

Вот точная механика бага (зафиксирована комментарием в `build_wc_img.cmd`,
строки 42–49, дата 2026-06-12):

> `--table` генерится по бинарям **ПРЕДЫДУЩЕЙ** сборки, а `--pack` пакует
> **свежие**. При изменении РАЗМЕРА кода первая сборка давала пак,
> рассинхронизированный с вшитой таблицей (мусорный `lba` на последней стадии
> загрузки, обрезанный трек уровня). Размер таблицы константен → фикспойнт
> достигается вторым проходом.

Разберём по шагам, почему наивный порядок `table → asm → pack` ломается:

1. Запускаем `--table`. Он читает `main1_play.bin` (и прочие бинари) **от прошлой
   сборки**, считает число чанков по их `stat().st_size`, пишет
   `main_pak_table.inc` с таблицей секторов.
2. `sjasmplus` собирает **новый** `main1_play.bin` (он же `Core.bin`/`ui_ovl.bin`/
   `loader_ovl.bin`). Если в код добавили 100 байт — файл может **вырасти и
   перейти границу 16К**, т.е. занять на один чанк (на одну страницу = 32 сектора)
   больше.
3. Запускаем `--pack`. Он читает **свежие** бинари, `parse_blocks` теперь видит на
   один чанк больше → в паке появляется лишняя 16К-страница, **все последующие
   страницы съезжают на 32 сектора вперёд**.
4. Но вшитая в прошивку таблица (`main_pak_table.inc` из шага 1) **ещё старая** —
   она указывает на старые секторные смещения.
5. Z80-загрузчик идёт по старой таблице, читает страницу N по старому `sector`, а
   там уже лежат байты страницы N±1 → **мусор**. Особенно заметно на *последних*
   страницах таблицы (накопленный сдвиг максимален) — отсюда «мусорный lba на
...
Ключевое наблюдение: **размер самой таблицы константен**. Число записей зависит
только от того, сколько чанков занимают бинари, а таблица — это `DEFB/DEFW`
фиксированной структуры на запись. Поэтому:

- **Первый проход** `table → asm`: таблица посчитана по старым бинарям, но
  собранный `asm` уже включает таблицу **нового** числа записей (`MAIN_PAK_PAGE_COUNT`).
  Бинари теперь имеют *финальный* размер.
- **Второй проход** `table → asm → pack`: `--table` пересчитывается по уже
  финальным бинарям → таблица совпадает с тем, что упакует `--pack`. Так как
  размер таблицы не меняется от прохода к проходу, бинари во втором проходе уже
  стабильны → **два прохода достаточно для фикспойнта**.

Это вшито в `build_wc_img.cmd` ровно так (фрагмент):

```bat
echo === make_main_pack table ===
python Source\OTHER\make_main_pack.py --table
echo === sjasmplus ===
%SJASMPLUS% Source\ASM\main.asm --syntax=ab --lst=Build\main.lst --sym=Build\zuma.sym

rem ВТОРОЙ ПРОХОД table+asm+pack (2026-06-12)
echo === make_main_pack table (pass 2) ===
python Source\OTHER\make_main_pack.py --table
echo === sjasmplus (pass 2) ===
%SJASMPLUS% Source\ASM\main.asm --syntax=ab --lst=Build\main.lst --sym=Build\zuma.sym
echo === make_main_pack pack ===
python Source\OTHER\make_main_pack.py --pack
echo === spgbld ===
%SPGBLD% -b spgbld_boot.ini Build\zuma_vdac2.spg
```

> ⚠️ **Грабля внутри грабли.** Соблазн «упаковать раньше» (поставить `--pack`
> после первого `sjasmplus`) **возвращает баг**: пак станет свежим, а вшитая в SPG
> таблица — из первого `--table` (по старым бинарям). Порядок жёсткий: финальный
> `--pack` идёт **только после второго `--table`+`asm`**, и `spgbld` (который
> вшивает `main_pak_table.inc` в SPG через `Core.bin`) — тоже **после** второго
> прохода. Пак и SPG обязаны выйти из одного фикспойнта.

### 36.5. `ZUMALVL.PAK`: трек на 2 страницы и сектор-выровненный TOC

Глава 30 описала формат `ZLVP` и рантайм-чтение. Со стороны сборки
(`make_level_pack.py`) добавляются два нетривиальных шага.

**1. Масштаб координат x8/5 на этапе пака (рефактор 1024×768).** Исходные треки
`track_l*_640.bin` остаются в 640×480-пространстве; пакер масштабирует **только**
координаты сэмплов, упакованных в PAK:

```python
TRACK_SCALE_NUM = 8     # x1.6 = 8/5
TRACK_SCALE_DEN = 5
# в scale_track_samples(): x = round(x * 8 / 5); y = round(y * 8 / 5)
```

Важно: **число сэмплов не меняется** — VDC-физика (скорость в сэмплах/кадр,
`CELL_SIZE` в сэмплах, tangent-байты) остаётся точно настроенной, меняется лишь
геометрия выдачи. Всё, что читает X/Y шара в рантайме (коллизия пули, лягушка,
killzone), должно жить в 1024-пространстве.

**2. Page-split трека > одной страницы.** Запись сэмпла трека —
**6 байт** в текущей сборке: `X:u16, Y:u16, tangent:u8, flags:u8`, плюс 2-байтовый
заголовок-счётчик. В одну 16К-страницу влезает:

```python
PAGE = 16384; TRACK_HDR = 2; TRACK_REC = 6
TRACK_SAMPLES_PER_PAGE = (PAGE - TRACK_HDR) // TRACK_REC   # = 2730
```

`pagesplit_track`: если `count > 2730`, режем **на границе сэмпла** (никакой
6-байтовый рекорд не пересекает `#C000`): chunk A = заголовок + первые 2730
сэмплов, добитый нулями до полных 16К; chunk B = остаток **без заголовка**. В
рантайме chunk A ложится на страницу `#06`, chunk B — на `TRACK_PAGE2 EQU #0F`.

Рантайм-чтение синхронно этому в `loader_resident.asm`, `VDC_ReadSampleAtHL`
(резидент в Core, зовётся каждый кадр из `VDC_SlotPos`):

```
TRACK_PAGE2        EQU #0F
TRACK_SPLIT_SAMPLE EQU 2730        ; (16384-2)/6
VDC_ReadSampleAtHL:
        LD   DE, TRACK_SPLIT_SAMPLE
        SBC  HL, DE                ; HL = t - split; CF=1 если t < split
        JR   C, .p1                ; .p1: страница #06, addr = #8000+2+t*6
        ; иначе страница #0F: addr = #8000 + (t-split)*6, заголовка нет
```

> ⚠️ **Согласованность 5↔6 байт.** Глава 30 (раздел 30.8) описывала *прежний*
> 5-байтовый формат сэмпла со `split = 3276 = (16384-2)/5`. Текущая сборка
> (рефактор 1024×768, опорная v069) перешла на **6-байтовую** запись, поэтому
> `TRACK_SPLIT_SAMPLE = 2730` и в пакере (`TRACK_SAMPLES_PER_PAGE`), и в
> `loader_resident.asm`. **Эти три числа (`TRACK_REC`, `TRACK_SAMPLES_PER_PAGE`,
> `TRACK_SPLIT_SAMPLE`) обязаны меняться вместе** — рассинхрон даёт ровно тот же
> класс бага, что в §36.3: сэмплы за стыком читаются с неверным шагом → «трек
> кривой/обрезанный».

**3. Сектор-выровненный TOC + встроенный verifier.** Каждая секция выровнена на
512 (`pad_to_sector`), `_verify()` распаковывает готовый пак и **байт-в-байт**
сверяет каждую секцию с исходником (включая нулевой паддинг хвоста). Пакер пишет
`pak_size.inc` с `EXPECTED_PAK_SIZE` — загрузчик матчит файл по имени **И точному
размеру**, чтобы не подхватить устаревшую копию.

### 36.6. `ZUMASND.PAK` / `ZUMAAUD.PAK`: своя пара table↔runtime граблей

`ZUMAAUD.PAK` тривиален — это сырой MOD (`shutil.copyfile`), пакер лишь пишет
`audio_pak_size.inc` (`EXPECTED_AUDIO_PAK_SIZE/SECTORS/FULL_SECS/TAIL_BYTES`) —
загрузчик стримит точное число байт из FAT-записи (глава 33).

`ZUMASND.PAK` (`make_sound_pack.py`) интереснее и несёт **свой аналог
рассинхрона table↔pack**: пакер генерит `sound_pak_size.inc` с
`GS_SfxPreloadTable` — список ID, секторных смещений и длин эффектов, которые
загрузчик `GS_LoadSfxPackWithTable` предзагружает в RAM General Sound:

```python
PRELOAD_IDS = [9, 10, 26, 20, 12, 2, 1, 34, 38,
               3, 4, 5, 6, 7, 24, 25, 28, 11, 21,
               23, 37, 32, 14, 19, 13]   # 25 звуков ≈ 353 КБ при GS 1 МБ
...
> (page `#05`), win-snapshot — в gameplay-оверлее (page `#04`). Три **разные
> физические страницы**, никогда не co-resident; +36 байт двигает только код
> *после* таблицы *внутри* page `#40` (путь загрузки уровня, не win-render).
> Таблица читается динамически (`LD B,GS_SFX_PRELOAD_COUNT`; per-entry `ADD HL,6`),
> поэтому 25 записей обрабатываются так же, как 19. **Урок: рост авто-таблицы
> опасен только если перепрыгивает границу слота (`#C000`/`#8000`) или двигает
> фикс-адрес; внутри-страничный сдвиг изолированного куска безопасен** — в отличие
> от §36.3, где двигалась *секторная раскладка всего пака*.

Дополнительный приём `make_sound_pack.py` (см. также главу 33.6): сэмпл не режут и
не фейдят, а **добивают unsigned-тишиной `0x80` до границы сектора** (+64 байта
запаса) — GS округляет длину FX вверх и доигрывает «добор» из своей RAM; раньше
там был мусор → шумовой хвост. `MIN_OUTPUT_SIZE = 1215488` держит размер FAT-записи
стабильным для частичных обновлений образа.

### 36.7. Инжект в образ: `inject_zuma_to_wc_img.py` и `test_wc.img`-харнесс

Финальная стадия — поместить четыре пака + SPG в FAT32-образ
(`inject_zuma_to_wc_img.py`). Это полноценный FAT32-райтер (`Fat32Image`:
аллокация кластеров, FAT-цепочки, LFN-записи, поиск свободных dir-слотов). Кладёт
в `/Games/Zuma Deluxe VDAC2/` пакет файлов (`prepare_host_package`):
`ZUMA_VD2.SPG`, `ZUMAMAIN.PAK`, `ZUMALVL.PAK`, `ZUMAAUD.PAK`, `ZUMASND.PAK`,
`SPGBLD.INI`, `ZUMA.SYM`, `README.TXT`.

Инжектор используется в **двух ролях**, и обе важны для pipeline:

1. **Хостовый `wc.img`** (in-place, для запуска в Unreal). Жёсткая защита от порчи
   образа: если `--base-img` и `--out-img` различаются — `REFUSING full image
   replacement`; если файла нет — `REFUSING to create a new image`. Сравнение
   путей строкой через `os.path.normcase(os.path.abspath(...))` (а не
   `samefile()`, который врёт на UNC `\\tsclient`, — грабля из главы 30.12). Перед
   запуском **закрыть Unreal.exe** на хосте, иначе `WinError 32` (файл занят).

2. **`Build/test_wc.img`** — образ для локального Z80-харнесса (глава 30.9).
   `build_wc_img.cmd` сначала `inject ... --base-img Build\test_wc.img --out-img
   Build\test_wc.img` (если файл есть), затем гоняет батарею Python-харнессов
   (`check_memory_map.py`, `test_levelsel_loader_walks.py`,
   `test_gameplay_level_load_ramg.py --level-index 0/1`, `test_score24_z80.py` и
   т.д.) — **на этом образе**, без хостовых циклов, до того как трогать настоящий
   `wc.img`. Только в самом конце — обновление хостового `wc.img` через
   PowerShell-обёртку (кириллица в UNC-пути собирается из код-поинтов).

Связь с §36.3: харнесс гоняется на образе, собранном из **финального** пака после
двойного прохода. Если бы пак собирался до фикспойнта, харнесс
`test_gameplay_level_load_ramg.py` отловил бы рассинхрон секторов локально
(мусорный RAM_G на загрузке уровня) — это и есть сетка безопасности, ловящая
грабли table↔pack **до** реального железа.

### 36.8. Ловушки (anti-patterns)

| Грабля | Симптом | Лечение |
|---|---|---|
| `--pack` сразу после первого `asm` | Пак свежий, вшитая таблица старая → мусорный lba на последних страницах, обрезанный трек | Двойной проход `table→asm→pack`; финальный `--pack` только после 2-го `--table`+`asm` |
| Рост *кода* (не данных) | `zx7`-данные не растут, код перепрыгивает 16К → пак +1 страница → сдвиг всех секторов | Тот же фикспойнт; не доверять «пак почти не изменился» |
| Принять рост пака за «регрессию геймплея» | Правка подтяжки «ломает загрузку уровня» | Сначала проверять секторную раскладку пака, потом физику (бисект показал: дело в табл↔пак) |
| Рассинхрон `TRACK_REC` / `TRACK_SAMPLES_PER_PAGE` / `TRACK_SPLIT_SAMPLE` | Сэмплы за стыком читаются с неверным шагом → кривой/обрезанный трек | Менять все три вместе (сейчас 6 / 2730 / 2730) |
| Новый `SND_X` без записи в `PRELOAD_IDS` | Звук молчит на реале, без ошибки (handle `#FF`) | Добавил `GS_PlaySfx` — добавь ID в `PRELOAD_IDS` |
| `samefile()` для проверки in-place образа | Ложный результат на UNC `\\tsclient` | Сравнивать `normcase(abspath(...))` строкой |
| Сборка/инжект при открытом Unreal.exe | `WinError 32` (файл занят) | Закрыть эмулятор перед `build_wc_img.cmd` |
| Положить boot-страницу (`#00/#05/#40/#41`...) в `ZUMAMAIN.PAK` | Загрузчик перетирает сам себя | `BOOT_PAGES` исключаются из `parse_blocks` |

### 36.9. Связано

- **Глава 30** — рантайм-чтение пака (RawPak/CMD17/LFN/двухфазная загрузка,
  непрерывность `LBA = PakLba + N`, page-split трека со стороны *чтения*).
- **Глава 33** — General Sound: стриминг `ZUMAAUD.PAK` (MOD) и `ZUMASND.PAK` (SFX),
  добивка сэмпла тишиной (33.6).
- `build_wc_img.cmd` — оркестратор сборки (двойной проход + батарея харнессов +
  in-place inject).
- `Source/OTHER/make_main_pack.py` — `parse_blocks`/`write_table`/`write_pack`,
  `BOOT_PAGES`.
- `Source/OTHER/make_level_pack.py` — TOC `ZLVP`, `scale_track_samples` (x8/5),
  `pagesplit_track`, встроенный `_verify`, `EXPECTED_PAK_SIZE`.
- `Source/OTHER/make_sound_pack.py` / `make_audio_pack.py` — SFX/MOD-паки,
  `GS_SfxPreloadTable`, `EXPECTED_*_PAK_*`.
- `Source/OTHER/inject_zuma_to_wc_img.py` — FAT32-райтер, `test_wc.img`-харнесс,
  in-place guard.
...
> **TL;DR.** Глава 33 ввела MOD+SFX через General Sound. Здесь — практический цикл
> и ловушки. (1) `GS_Detect` командой `#F3` ставит `GS_Present`; нет карты → таймаут
> `GS_WAIT_TIMEOUT=#FFFF`, `CF=0`. (2) **Самая дорогая ловушка — «звук молчит без
> ошибки»**: `GS_PlaySfx` играет сэмпл, ТОЛЬКО если он *предзагружен* в RAM карты;
> список — `PRELOAD_IDS` в `make_sound_pack.py` → `GS_SfxPreloadTable` →
> `GS_LoadSfxPackWithTable`; не предзагруженный звук имеет `handle=#FF` и `GS_PlaySfx`
> молча выходит. Добавил `CALL GS_PlaySfx` с новым `SND_X` — **обязан** внести `X` в
> `PRELOAD_IDS`. (3) Музыка стримится в карту покадрово, и именно эти тики двигают
> анимацию загрузки. (4) SFX с питчем через `GS_PlaySfxNote`.

### 37.1. Детект карты: `GS_Present`

`GS_Detect` (`ts-dos.asm`) обнуляет состояние и шлёт команду `#F3` (reset/flags)
через `GS_SendCommandOverlay`. Шина GS отдельная от SD/FT (порты `#BB` команд,
`#B3` данных). Без карты порт не подтверждает bit0 → петля крутит все `#FFFF`
итераций → `CF=0`, `GS_Present=0`. Все звуковые вызовы дальше защищены этим
флагом: при `GS_Present=0` они — безопасный no-op (важно для конфигурации без
карты, см. главу 38 про реал-vs-эмулятор).

### 37.2. Ловушка «звук молчит»: предзагрузка обязательна

`GS_PlaySfx` (`loader_resident.asm`) не «проигрывает файл с карты» — он триггерит
**уже загруженный в RAM карты** сэмпл по его handle. Цепочка загрузки:

1. `make_sound_pack.py` → массив `PRELOAD_IDS` (какие ID грузить) → генерит
   `GS_SfxPreloadTable` в `sound_pak_size.inc` (по 6 байт на запись: id, sector,
   full_secs, tail).
2. `GS_LoadSfxPackWithTable` (`ts-dos.asm`) на boot: для каждой записи команда
   `#38` (Load FX), карта возвращает handle → пишем в `GS_SfxHandles[id]`.
3. `GS_PlaySfx`: читает `GS_SfxHandles[id]`; если `#FF` (не загружен) — `.done`
   (тихий выход). Иначе — `#98` (Play FX).

**Грабля:** добавляешь в код `LD A,SND_X / CALL GS_PlaySfx`, собирается без
ошибок, но на реале — тишина, потому что `X` не в `PRELOAD_IDS` → `handle=#FF`.
Так и случилось с озвучкой Win/Lose: 6 звуков (`earthquake`, `warning1`, `pop`,
`chant2`, `chant8`, `chant14`) были в коде, но не в preload — молчали, пока их не
добавили в `PRELOAD_IDS`. Правило: **новый вызываемый `SND_*` → обязательно в
`PRELOAD_IDS`.**

Бюджет: карта 1 МБ; 25 предзагруженных звуков (raw 8-бит моно 11025 Гц) ≈ 353 КБ,
запас ~687 КБ — partial-fail исключён. Размеры считаются по TOC пака (формат
`ZSPD`); крупные — `chant8` ~50 КБ, `earthquake` ~40 КБ.

### 37.3. Стрим музыки MOD в карту — и почему он двигает анимацию загрузки

`OVL_GS_InitAndStartMenuMusic` грузит `ZUMAAUD.PAK` и стримит MOD в RAM карты
посекторно: `GS_StreamAudioCacheToDevice` гоняет `EXPECTED_AUDIO_PAK_FULL_SECS`
секторов, на каждом — `GS_StreamProgressTick`. Тик каждые 32 сектора вызывает
`BootLoadingTickSafe` (перерисовка экрана загрузки). **Именно эти тики продвигают
кадры бут-анимации** — что становится западнёй без карты (стрима нет → анимация
не получает тиков; разбор и фикс — глава 38). Стрим работает на отдельной шине,
между секторами отдаёт `BootBusOwner`, так что FT-рисование на SPI не конфликтует.

### 37.4. SFX с питчем: `GS_PlaySfxNote`

`GS_PlaySfx` играет с нотой по умолчанию `GS_SFX_NOTE=53` (≈11025 Гц payload).
Для питча — `GS_PlaySfxNote` (A = sound id, C = нота = `GS_SFX_NOTE + полутона`).
Примеры из движка:

- **Взрыв матч-3** (`VDC.asm`): ОДНОВРЕМЕННО `ballsdestroyed{N}` (N — уровень
  комбо 0..4, через `ADD A,SND_BALLSDESTROYED1`) И `chime1` с питчем `2·combo`
  полутонов (`GS_PlaySfxNote`). Это не «вариация + отдельный звон» (как ошибочно
  в Wiki), а два звука вместе.
- **Lose-секвенция оригинала** (`main.asm`): `earthquake` при достижении черепа,
  `warning1`-сирена при приближении, `pop` с растущим питчем (+1 полутон/шар,
  кламп +24) на каждый всосанный шар. HD-ремейк это упростил до тишины — для
  аутентичности ориентир оригинал, не ремейк (см. `Sounds/Zuma Sounds VERIFIED.md`).

### 37.5. Почему звук GS не включался на реале, но играл в эмуляторе (порядок записи в FIFO)

Классический реал-vs-эмулятор именно при загрузке музыки: в эмуляторе (Unreal)
звук GS включался, на реальном железе (MultiSound/ЮзикС, GS-совместимый) —
**тишина**. Корень — **порядок записи байта в FIFO General Sound** (порт данных
`#B3`):

- **Было (неверно):** пишем байт в `#B3`, *потом* ждём `bit7` (готовность места
  в FIFO).
- **Эталон WC gsplayer (`GSLOAD`/`TE0`):** *сначала* ждём `bit7=0` (в FIFO есть
  место), *потом* `OUTI` (записываем байт).

Запись без предварительной проверки места **переполняет FIFO реального GS** —
байты теряются, модуль/сэмпл приходит битым, команда воспроизведения по сути
работает по мусору → тишина. Эмулятор моделирует FIFO «бездонно» (приём
мгновенный), поэтому неверный порядок там не наказывается — отсюда «в эмуляторе
играет, на реале нет». Правило (универсальное для любого FIFO-устройства на
шине): **сначала ждать готовность (`bit7=0` / флаг места), потом писать данные —
никогда не наоборот.** Тот же принцип у `GS_StreamBufferDE` (стрим модуля) и у
SFX-загрузки.

Сопутствующее — **GS на реале не успевал включить песню.** После стрима модуля
(`ZUMAAUD.PAK`) команда `MODPLAY` / `GS_PlayMenuMusic` приходила **раньше, чем
устройство успевало переварить загрузку**, и музыка не запускалась (в эмуляторе
всё мгновенно — играло). Лечение — пауза-settle `GS_PostLoadSettle` между «Load
...
`WaitFlush` ловит фолт копроцессора (`REG_CMD_READ=0xFFF`), но НЕ ловит
не-фолтовое вечное «занято». Главный такой сценарий — `CMD_INFLATE`, которому
недодали хвост zlib-потока (рассинхрон `*_size.inc` с паком): копроцессор ждёт
данных вечно, `READ≠WRITE`, фолта нет. Команды GS (`GS_SendCommand/DataResident`)
имеют `GS_WAIT_TIMEOUT` и при таймауте возвращают `CF=0` — то есть сами по себе не
виснут даже при мёртвой/отсутствующей карте, но игнорировать их `CF` нельзя.

### Ловушки (свод)

- Вызываемый `SND_*` без записи в `PRELOAD_IDS` → `handle=#FF` → тишина без ошибки.
...
> **TL;DR.** Глава 34 уже разбирала расхождения SPI/SD/GS/init. Здесь — новые
> классы, найденные в сессии 2026-06. (1) **`REG_INT_FLAGS` — clear-on-read**: на
> реальном FT812 чтение регистра очищает его, запись-очистка — no-op; в эмуляторе
> наоборот → переходный чёрный кадр «съедал» событие `INT_SWAP` следующего кадра →
> **зависали ВСЕ переходы между экранами на реале**, в эмуляторе работало. (2)
> **Привязка визуала к длительности опционального этапа**: бут-анимация двигалась
> тиками стрима в GS-карту; без карты этап мгновенный → анимация пропадала.
> Правильно — привязывать к *реальному прогрессу* оставшегося pipeline, не к
> времени/циклам. (3) Копроцессор-фолты/таймауты, FIFO 4 КБ, `GSType=NONE`.

### 38.1. `REG_INT_FLAGS` — семантика clear-on-read (зависание всех переходов)

По даташиту FT81x `REG_INT_FLAGS` **очищается самим актом чтения** хоста; запись
в него — no-op. Эмулятор (Unreal/VDAC2-модель) реализует иначе: чтение НЕ
очищает, чистит запись. Эта разница долго была невидима, пока не появился
переходный чёрный кадр между сценами.

`DrawBlackTransitionFrame` (`shared_render.asm`) свопает чистый чёрный DL и ждёт
его показа, читая `REG_INT_FLAGS` в своей петле `.wait_black`; перед этим он же
вызывает `MenuSwapFrame`, который тоже ждёт `INT_SWAP`. На реале **каждое чтение
очищает флаг**, поэтому переходный кадр потребляет ДВА события `INT_SWAP` (своё +
в `.wait_black`), а производит ОДНО → первый `MenuSwapFrame` новой сцены ждёт
событие, которого уже не будет, и виснет в `.wait_int` **навсегда**. Симптом:
зависают **все** переходы между экранами (выход из паузы в меню, главный→Gauntlet
и т.п.), а сам геймплей не виснет (там баланс событий 1:1). В эмуляторе чтение не
очищало флаг — стейл-бит проскакивал петлю, и баг не воспроизводился.

**Фикс:** в сценах ждать **level-based** `REG_DLSWAP == 0` (display engine забрал
своп на границе кадра — тот же момент, что `INT_SWAP`, но идемпотентный), а не
edge-based `INT_FLAGS` (как в hw-проверенном v067). Правило: **clear-on-read
регистр нельзя поллить из двух независимых мест; для ожидания между петлями
использовать level-based сигнал.**

### 38.2. Привязка визуала к длительности опционального этапа (бут-анимация)

Бут-анимация логотипа продвигалась тиками экрана загрузки, а основную массу
тиков (десятки-сотни) давал **стриминг аудио-пака в GS-карту**
(`GS_StreamProgressTick`, глава 37). Когда карта отключена (`GSType=NONE` →
`GS_Present=0`), этот этап мгновенный — анимация не получает тиков и либо
застревает на кадре 0, либо исчезает.

Изначально это пытались чинить **пустыми циклами** (крутить `BootLoadingTickSafe`
ради кадров) — но тогда прогресс-бар стоит: загрузка не идёт, время мёртвое. Это
неверный путь.

**Правильное решение (паттерн):** декоративную/информативную анимацию привязать к
**уже идущим progress-событиям** оставшегося pipeline, а не к времени или
delay-счётчику. В no-GS ветке фазы вычисляются из `BootProgressPx` во время
`LoadMainPack`: диапазон прогресса `95..192` отдан первой анимации
(`frame=(progress-95)/9`, clamp 0..10), `192..255` — второй (`frame=1+(progress-192)/16`),
а `OVL_GS_LoadGameplaySoundsMaybe` при `GS_Present=0` просто возвращается,
оставляя переключение фаз ветке `BootAnimAdvanceNoGs`. Тогда медленный/быстрый
носитель не ломает порядок: новый тик загрузки → новый кадр. GS-путь оставлен
старым (tick/delay-based, его двигает стрим).

Универсальный урок: **при исчезновении опциональной длинной фазы (GS/сеть/
устройство) не эмулировать её пустым wait-loop — переразложить визуальные фазы по
реальным progress-событиям оставшегося конвейера.**

### 38.3. `GSType=NONE` на эмуляторе

В `Unreal.ini` секция `[SOUND]`, ключ `GSType` (BASS/Z80/NONE). `GSType=NONE`
отключает эмуляцию GS — `GS_Detect` уходит в таймаут, `GS_Present=0`, звук — тишина,
и работают no-GS ветки. Это удобный режим для теста поведения без карты прямо в
эмуляторе (без реального железа). Важно понимать: какой именно ветке (GS / no-GS)
соответствует то, что видишь — иначе можно долго править мёртвую ветку.

### 38.4. Копроцессор-фолты, таймауты, FIFO

- `WaitFlush` корректно ловит фолт (`REG_CMD_READ` = `0xFFF`), но НЕ ловит
  не-фолтовое вечное «занято» (`READ≠WRITE` без фолта) — например `CMD_INFLATE` с
  недоданным/невыровненным хвостом ждёт данных вечно.
- Вызывающие часто игнорируют `CF` от `WaitFlush` → после фолта нет
  восстановления (`FT_CMD_RESET` — только в `Init_Video`), FIFO не дренируется,
  кадры слепо вливаются в полный буфер → экран замерзает (Z80 жив).
- FIFO копроцессора 4096 байт (макс `CMDB_SPACE` 4092). Кадр игры ~3920 байт
  помещается БЕЗ фолта; защита от переполнения (`ZL_CmdOverflow`) есть только в
  gameplay-кадре, у меню/level-select size-guard'а нет — кадр >4092 переполнит
  FIFO даже на пустом.
- `CMD_INFLATE` требует выравнивания данных до 4 байт; невыровненный хвост на
  реальном чипе заставляет копроцессор ждать вечно.

Все эти классы **в эмуляторе не воспроизводятся** (фолтов нет, FIFO «бездонный»,
тайминги идеальные).

### 38.5. Чем харнесс НЕ ловит реальные расхождения

Z80-харнессы (`profile_dual_chain_perf.py`, `full_stack_trace.py`) выполняют Z80-код
и читают/пишут память, но **не моделируют**: clear-on-read семантику `INT_FLAGS`,
фолты/таймауты копроцессора, реальные тайминги развёртки, поведение
отсутствующего/«болваночного» GS на шине. Поэтому баг переходов (§38.1) в харнессе
не воспроизводился, как и в эмуляторе — он чисто аппаратный. Вывод: **аппаратные
расхождения харнесс/эмулятор не покажут — их ловят только на реальном железе или
анализом даташита**, а харнесс хорош для логики (физика цепи, загрузка, бюджеты).

### Ловушки (свод)

- `REG_INT_FLAGS` clear-on-read: не поллить из двух мест; ждать `REG_DLSWAP==0`.
- Анимация/прогресс не привязывать ко времени опционального этапа — к progress-событиям pipeline.
- `WaitFlush` не ловит не-фолтовое вечное «занято»; игнор `CF` → нет восстановления после фолта.
...
> (`BallChain.c`) и реализована в slot-модели `VDC.asm`. **PULL** (цвета по краям
> стыка совпадают) — магнитный разгон скорости подтяжки `vp` 0.4→10 сэмпл/кадр;
> **CATCH-UP** (цвета разные / нет фронта) — фронт стоит, хвост догоняет темпом
> цепи; слияние — отдача `vp/2.5` + клик `SND_BALLCLICK1`. Пять дефектов
> «квантованной» модели давали рывки; плюс Z80-ловушка `PUSH AF/POP AF` и
> win-регрессия от доспавна. Все разобраны ниже.

### 39.1. Slot-модель цепи

Цепь — массивы `Slots[]` (цвет 0..5 или gap-маркер `#FE`=STOP / `#FD`=CASCADE),
`Offsets[]` (знаковый сдвиг шара в пикселях), `Shot2[]` (флаг отложенного матча).
Позиция шара на треке: `t = (HSA − i)·CELL + HSub + Offsets[i]`, где `CELL=32`
сэмпла, `HSA`/`HSub` — голова и суб-позиция, `i` — индекс слота. Глобальный темп
`VDC_GLOBAL_SPEED_FACTOR=2` (два тика `SpeedAdvance` на кадр). Удаление слота
(`VDC_RemoveSlotAt`) сдвигает индексы — а значит `t` всех слотов за точкой
удаления прыгает на +CELL (ключевой нюанс для §39.3).

### 39.2. Механика по референсу

`VDC_GapJunctionUpdate` раз в кадр классифицирует ближайший стык, который закроет
`VDC_DoGapStep`: **0** — гэпов нет; **1 (PULL)** — цвета краёв рана совпадают
(магнетизм одного цвета, как в референсе); **2 (CATCH-UP)** — цвета разные или
нет фронт-сегмента.

- **PULL:** `vp` разгоняется `+VDC_PULL_ACCEL/10` за кадр до `VDC_PULL_MAX`
  (0.4 → 10 сэмпл/кадр), фронт подтягивается к хвосту; на слиянии — отдача
  заднего сегмента `vp/2.5` (BALL_WEIGHT_RATIO) + клик `SND_BALLCLICK1`, разгон
  сбрасывается (`VDC_GapMergeCheckRecoil`).
- **CATCH-UP:** фронт-сегмент СТОИТ (без `HsaDec`), хвост догоняет **темпом цепи**
  (`speed_x100/5`, уже с глобальным ×2); rear-компенсация offsets, доезд декеем.
...
1. **Двойной ×2 темпа догона.** `speed_x100/5` в x10-единицах **уже** содержит
   глобальный ×2 (два тика SpeedAdvance/кадр). Лишний `ADD HL,HL` гнал догон вдвое
   быстрее цепи → rear-comp упиралась в кламп −CELL → телепорты на 32 px.
2. **Кап компенсации в CELL срезал недотаявший остаток.** При разгоне PULL
   следующий шаг приходит раньше, чем дотаял прошлый +CELL; кап в CELL терял
   остаток → микро-рывок головы каждый шаг. Фикс — **кап 2×CELL** (перенос
   остатка во вторую клетку; баланс притока CELL/шаг и декея держит offset в
   пределах 2·CELL).
3. **Усечённый декей `vp/10` терял до 0.9 px/кадр.** Каденция шагов копит сэмплы
   точно (×10), а декей с целочисленным усечением отставал → к слиянию копился
   хвост ~13 px, скорость рушилась 10→1 px/кадр. Фикс — **дробный накопитель
   декея** `VDC_GapDecAcc` (остаток 0..9): приток и отток сходятся точно.
4. **Сброс темпа при матче рвал доезд финальной клетки.** Свежий матч сбрасывал
   `vp`/аккумулятор, и недотаявшие компенсации прошлого PULL ползли базовым
   1 px/кадр («летит-замирает»). Фикс — **удержание `VDC_GapTempo`, пока живы
   положительные комп-офсеты** (флаг `VDC_GapPosLeft`); матч сбрасывает `vp`, но
   не темп.
5. **Мгновенное обнуление offsets матч-рана прыгало взрывом.** Шары/взрывы
   рендерятся ещё `VDC_EXPLOSION_FRAMES` кадров и обязаны доезжать; мгновенный 0
...
хвост) — не видно; в «зелёной фазе» (шкала полна, спавн выключен) дыр несколько →
рывки. `VDC_GapRearComp` компенсирует все `j` до конца цепи (маркеры
включительно); «задний сегмент есть» = есть хоть один живой шар за стыком
(`VDC_GapRearExists`, скан мимо маркеров), иначе чистая уборка без freeze.

### 39.4. Z80-ловушка: `PUSH AF / … / POP AF` затирает флаги сравнения

Кламп компенсации сначала был написан так: `ADD A,CS / PUSH AF / XOR #80 /
CP n / POP AF / JR C,save`. Это **сломано**: `POP AF` восстанавливает флаги,
сохранённые от `ADD`, **затирая результат `CP`** — `JR C` срабатывает не по `CP`,
а по `ADD`. С капом CELL он давал случайно верный результат, а с капом 2×CELL
дефект вскрылся. Диапазон сумм [0..96] позволяет простое беззнаковое `CP n` без
сохранения флагов. Правило: **никогда не полагаться на флаги после `POP AF` —
он восстанавливает СТАРЫЕ флаги, а не результат промежуточного `CP`.**

### 39.5. Win-регрессия от доспавна — и урок про тестовое состояние

Догон (CATCH-UP) съедает клетку хвостового сегмента, а HSub-гейт выдачи отдаёт
максимум 1 шар/клетку движения — без доспавна выдача отстаёт, цепь «рвётся» у
жерла. Поэтому на шаге CATCH-UP делается `VDC_TrySpawn_NoHsubGate` (`.dgs_backfill`).

В отладке это **ложно обвинили** в поломке win: проба с `gaugeFull=0`
показывала, что доспавн плодит шары, и цепь не пустеет. Но при **победе шкала
полна** (`gaugeFull=1`), а `VDC_TrySpawn_NoHsubGate` первым делом проверяет
`GaugeFull` и выходит → при win доспавн = no-op, цепь штатно пустеет, win не
ломается. Проба была нерепрезентативна; удаление доспавна сломало хвост в обычной
игре. Урок: **win-сценарии тестировать ТОЛЬКО при `gaugeFull=1`; не делать вывод
о регрессии из теста с неверным игровым состоянием.**

### 39.6. Верификация двумя моделями

Механика проверяется и z80-харнессом (`test_gap_pull_z80.py`), и Python-зеркалом
(`vdc_visual_emulator.py`) — они сходятся бит-в-бит (PULL закрывает 3 гэпа за
20 кадров в обоих). Тест держит инварианты: дельты головы в [-12..0] (без рывков
вперёд), мультигэп-сценарий — дельты всех шаров в [-12..+3], концевые маркеры
зелёной фазы убираются без freeze, тройка опустошает цепь до 0 (win-условие).

### Ловушки (свод)

- Темп догона: `speed/5` уже с глобальным ×2 — не умножать ещё раз.
- Кап компенсации = 2×CELL (перенос остатка), не CELL.
- Декей — дробный накопитель, не целочисленное усечение.
- Темп удерживать, пока живы положительные комп-офсеты (доезд финальной клетки).
- offset матч-рана обнулять при финализации взрыва, не мгновенно.
- rear-comp до конца цепи (несколько дыр в зелёной фазе).
...
> пункт — короткая формулировка + ссылка на главу с разбором. Пробежать перед
> коммитом/сборкой.

### 40.1. Матрицы и координаты FT812

- **`CMD_SCALE` с нецелым множителем — запрещён**: копроцессор хранит инверсную
  матрицу с усечением (1/1.6 → 159/256 → дрейф). Только запечённые
  `BITMAP_TRANSFORM` (160/256). Целые масштабы (×2, ×4) точны. → гл. 12, 35.
- **Точная арифметика констант, не шорткаты**: `cos·1.25` вместо `cos·323/256`
  дал 158/256 → масштаб 1.6203 → срез спрайта. → гл. 35.4.
- **Окно отрисовки при нецелом апскейле = `ceil(W·8/5)`, не `round`**: `round`
  вниз (195 вместо 195.2) субпиксельно срезает значимый край (тарелка лягушки
  справа/снизу) даже при точной матрице. → гл. 35.4.
- **`VERTEX2F` 15-бит (макс 1023.94 px), `VERTEX2II` 9-бит (макс 511)**
  переполняются на 1024×768; крупные координаты — через `Cell+Vertex2f`. → гл. 35.5.
- **`BILINEAR+BORDER`** даёт полупрозрачный край → чёрная вертикальная линия на
  стыке при скролле неба; для тайлинга `NEAREST` встык или `REPEAT` (степень
  двойки). → гл. 35.6.
- **Нецелый `NEAREST ×1.6` рвёт тонкие детали** (шрифты, логотип ZX Evolution):
  неравномерное дублирование пикселей (8/5). Тонкая графика — целый ×2 или
  нативный кегль. → гл. 35.8.
...
- **`FT_Scissor` / `Cell` / `ColorA` клобают BC/DE** — координаты грузить ПОСЛЕ
  них, не до. → гл. 20.3, 27.4.
- **Двойной swap запрещён** (`CMD_SWAP` + ручной `REG_DLSWAP` = «кадр назад»);
  vsync-ожидание перед отправкой кадра. → гл. 17.
- **FIFO копроцессора 4096 байт** — кадр > 4092 переполняет; size-guard есть не
  везде (gameplay да, меню нет). → гл. 38.4.
- **`WaitFlush` ловит фолт, но НЕ не-фолтовое вечное «занято»** (`CMD_INFLATE` с
  недоданным/невыровненным хвостом ждёт вечно). → гл. 38.4.
...
- **Цикл `make_main_pack.py --table ↔ --pack`**: таблица вшивается по бинарям
...
- **Юникод в python-принтах** под cp1251-консолью валит сборку из фонового
  шелла — `PYTHONIOENCODING=utf-8`.

### 40.5. Звук (General Sound)

- **Вызываемый `SND_*` без записи в `PRELOAD_IDS`** → `handle=#FF` → тишина без
  ошибки. → гл. 37.2.
- Тики стрима музыки двигают бут-анимацию; без карты их нет → не привязывать
  анимацию к этому этапу. → гл. 37.3, 38.2.
- `GS_PlaySfxNote` для питча (C=нота); матч-3 = `ballsdestroyed{combo}` +
  `chime1{pitch}` одновременно. → гл. 37.4.
- **Порядок записи в FIFO GS**: сначала ждать `bit7=0` (место), потом `OUTI` —
  иначе переполнение реального FIFO → битый модуль → тишина (в эмуляторе FIFO
  «бездонный», работало → звук есть в эмуляторе, нет на реале). Плюс
  `GS_PostLoadSettle` между Load Module и Play. → гл. 37.5.

### 40.6. Z80-идиомы

- **`PUSH AF / … / POP AF` затирает флаги `CP`** — `POP AF` восстанавливает СТАРЫЕ
  флаги (от `ADD`), а не результат промежуточного `CP`. → гл. 39.4.
- **`DEFB` vs `DEFW` при поках в тестах**: писать `HSA` (DEFB) как `sw` (16-бит)
  затирает соседний байт. → опыт `test_gap_pull_z80.py`.
- **Насыщение счётчиков**: `INC` с saturate, чтобы не завернуть (например
  `BallsSpawned` не должен wrap'нуть обратно в fast-фазу).
- **1-байтовые delay-счётчики** (`BootAnimDelay`) — не превышать 255.

### 40.7. Реал vs эмулятор (харнесс/Unreal НЕ ловят)

- **`REG_INT_FLAGS` clear-on-read** на реале (чтение очищает; запись — no-op);
  не поллить из двух мест → зависание переходов. Ждать `REG_DLSWAP==0`. → гл. 38.1.
- **Фолты копроцессора, FIFO, тайминги** — в эмуляторе не воспроизводятся. → гл. 38.4.
- **GS-болванка / `GSType=NONE`**: понимать, какая ветка (GS/no-GS) работает, иначе
  правишь мёртвую. → гл. 38.3.
- **Анимацию/прогресс привязывать к реальным progress-событиям pipeline**, не к
  времени/delay/пустым циклам (иначе ломается при отсутствии опц. этапа). → гл. 38.2.

### 40.8. Тестирование и отладка

- **Win-сценарии тестировать ТОЛЬКО при `gaugeFull=1`** — иначе тест в неверном
  состоянии даёт ложный вывод о регрессии (реальный урок: зря удалили доспавн). → гл. 39.5.
- **Не делать вывод о причине из теста с неверным игровым состоянием** — сначала
  убедиться, что воспроизводишь именно целевой сценарий.
- **Две модели (z80-харнесс + Python-зеркало)** должны сходиться бит-в-бит — это
  ловит расхождение реализации. → гл. 39.6.
