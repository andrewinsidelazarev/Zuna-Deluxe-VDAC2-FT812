...
OUT = Path("zuma_gs4_1mb_gssafe.mod")
REAL_ORDER_COUNT = 24
NOTE_SHIFT = 0
DOWNSAMPLE = 1
PER_INSTR_DOWNSAMPLE = {
    # GS-safe: keep ProTracker periods within C-1..B-3 by baking one octave
    # of flute pitch into sample data instead of using 4th-octave periods.
    4: 2,
    5: 2,
    7: 2,
    8: 2,
...
def cell(instruments, note=0, ins=0, eff=0, param=0):
    ...
    if note:
        rel = instruments[ins - 1][5] if 1 <= ins <= len(instruments) else 0
        ds = instruments[ins - 1][6] if 1 <= ins <= len(instruments) else 1
        idx = max(0, min(35, note - 37 + rel + downsample_pitch_compensation(ds) + NOTE_SHIFT))
        period = PERIODS[idx]
    return bytes([
