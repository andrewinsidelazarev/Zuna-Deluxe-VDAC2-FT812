...
def downsample_pitch_compensation():
...
def read_xm():
    ...
    for ii in range(1, 32):
        if ii > inst_count:
            instruments.append((b"", 0, 0, 0, 1, 0))
        ...
        pos += ih
        if not ns:
            instruments.append((name, 0, 0, 0, 1, 0))
            sample_data.append(b"")
            continue
        sh = pos
        ...
        data8 = downsample_to_s8(pcm, DOWNSAMPLE)
        frame_div = (2 if is16 else 1) * DOWNSAMPLE
        ls = loop_start // frame_div
        ll = loop_len // frame_div
        if not (typ & 3) or ll < 2:
            ls, ll = 0, 2
        instruments.append((name, len(data8), volume, ls, max(1, ll), rel_note))
...
def cell(instruments, note=0, ins=0, eff=0, param=0):
    ...
    if note:
        ...
        idx = max(0, min(len(PERIODS) - 1, note - 37 + rel + downsample_pitch_compensation() + NOTE_SHIFT))
        period = PERIODS[idx]
    return bytes([
...
def build():
    ...
    for name, length, vol, loop_start, loop_len, _rel_note in instruments[:31]:
